...
void SX126xIoInit(void) {
  ...
  ret = gpio_export(LORA_RADIO_DIO1_PIN);
  if (ret < 0) {
    log(ERROR, "gpio (%d)_export failed\n", LORA_RADIO_DIO1_PIN);
    return;
  }
...
#if defined(USE_RADIO_DEBUG)
  GpioInit(&DbgPinTx, RADIO_DBG_PIN_TX, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL,
           0);
  GpioInit(&DbgPinRx, RADIO_DBG_PIN_RX, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL,
           0);
#endif
}
...
void SX126xWaitOnBusy(void) {
  ...
  while (gpio_get_value(LORA_RADIO_BUSY_PIN) == 1) {
    int i = 0;
    if (i >= 200000) {

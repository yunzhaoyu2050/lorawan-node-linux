...
void SX126xCheckDeviceReady(void) {
  if ((SX126xGetOperatingMode() == MODE_SLEEP) ||
    ...
    SX126xWakeup();
    // Switch is turned off when device is in sleep mode and turned on is all
    // other modes
    SX126xAntSwOn();
  }
  SX126xWaitOnBusy();
...
RadioPacketTypes_t SX126xGetPacketType(void) { return PacketType; }
...
void SX126xSetTxParams(int8_t power, RadioRampTimes_t rampTime) {
  ...
  //            power = -17;
  //        }
  //        SX126xWriteRegister( REG_OCP, 0x18 ); // current max is 80 mA for
  //        the whole device
  //    }
  //    else // sx1262
  {
    // WORKAROUND - Better Resistance of the SX1262 Tx to Antenna Mismatch, see
    // DS_SX1261-2_V1.2 datasheet chapter 15.2 RegTxClampConfig = @address
    // 0x08D8
...
void SX126xSetModulationParams(ModulationParams_t *modulationParams) {
  ...
  case PACKET_TYPE_GFSK:
    n = 8;
    tempVal = (uint32_t)(32 * ((double)XTAL_FREQ /

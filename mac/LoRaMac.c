...
typedef struct sLoRaMacCtx {
  ...
  /*
   * Limit of uplinks without any donwlink response after a the first frame with
   * set ADRACKReq bit before the trying to regain the connectivity.
   */
...
/*!
 * Computes next 32 bit downlink counter value and determines the frame counter
 * ID.
 *
 * \param[IN]     addrID                - Address identifier
 * \param[IN]     fType                 - Frame type
 * \param[IN]     macMsg                - Data message object, holding the
 * current 16 bit transmitted frame counter \param[IN]     lrWanVersion -
 * LoRaWAN version \param[OUT]    fCntID                - Frame counter
 * identifier \param[OUT]    currentDown           - Current downlink counter
 * value
 *
 * \retval                              - Status of the operation
 */
static LoRaMacCryptoStatus_t
GetFCntDown(AddressIdentifier_t addrID, FType_t fType,
            LoRaMacMessageData_t *macMsg, Version_t lrWanVersion,
...
/*!
 * \brief Gets the maximum application payload length in the absence of the
 * optional FOpt field.
 *
 ...
 * \param [IN] macIndex     The index of the payload where the MAC commands
 * start \param [IN] commandsSize The size of the MAC commands \param [IN] snr
 * The SNR value  of the frame \param [IN] rxSlot       The RX slot where the
 * frame was received
 */
static void ProcessMacCommands(uint8_t *payload, uint8_t macIndex,
                               uint8_t commandsSize, int8_t snr,
                               LoRaMacRxSlot_t rxSlot);
 ...
 */
static void RemoveMacCommands(LoRaMacRxSlot_t rxSlot, LoRaMacFrameCtrl_t fCtrl,
                              Mcps_t request);
 ...
 * wave mode \param [IN] frequency   RF frequency to be set. \param [IN] power
 * RF output power to be set. \retval status          Status of the operation.
 */
LoRaMacStatus_t SetTxContinuousWave(uint16_t timeout, uint32_t frequency,
                                    uint8_t power);

/*!
 * \brief Converts a second based Rejoin Cycle base in the from timer module
 * required format.
 *
 ...
 * \retval status          Status of the operation.
 */
static bool ConvertRejoinCycleTime(uint32_t rejoinCycleTime,
                                   uint32_t *timeInMiliSec);
 ...
 *          \ref LORAMAC_STATUS_PARAMETER_INVALID,
 */
LoRaMacStatus_t DetermineFrameType(LoRaMacMessageData_t *macMsg,
                                   FType_t *fType);
...
/*!
 * \brief Checks if the retransmission should be stopped in case of a
 * unconfirmed uplink
 *
...
/*!
 * \brief Checks if the retransmission should be stopped in case of a confirmed
 * uplink
 *
 ...
 *                             with the LinkAdrReq.
 */
static bool CheckForMinimumAbpDatarate(bool adr, ActivationType_t activation,
                                       bool datarateChanged);
 ...
 */
static bool LoRaMacHandleResponseTimeout(TimerTime_t timeoutInMs,
                                         TimerTime_t startTimeInMs);
...
static void OnRadioTxDone(void) {
  ...
  if ((MacCtx.MacCallbacks != NULL) &&
      (MacCtx.MacCallbacks->MacProcessNotify != NULL)) {
...
static void ProcessRadioTxDone(void) {
  ...
  if (MacCtx.NodeAckRequested == true) {
    ...
    TimerSetValue(&MacCtx.RetransmitTimeoutTimer, 0,
                  MacCtx.RxWindow2Delay + phyParam.Value);
  ...
  txDone.ElapsedTimeSinceStartUp =
      SysTimeSub(SysTimeGetMcuTime(), Nvm.MacGroup2.InitializationTime);
...
static void ProcessRadioRxDone(void) {
  ...
  case FRAME_TYPE_JOIN_ACCEPT:
    ...
    // Abort in case if the device isn't joined yet and no rejoin request is
    // ongoing.
    if ((Nvm.MacGroup2.NetworkActivation != ACTIVATION_TYPE_NONE) &&
    ...
    macCryptoStatus = LoRaMacCryptoHandleJoinAccept(
        JOIN_REQ, SecureElementGetJoinEui(), &macMsgJoinAccept);
    ...
    if (LORAMAC_CRYPTO_SUCCESS == macCryptoStatus) {
      ...
      Nvm.MacGroup2.MacParams.Rx1DrOffset =
          macMsgJoinAccept.DLSettings.Bits.RX1DRoffset;
      Nvm.MacGroup2.MacParams.Rx2Channel.Datarate =
          macMsgJoinAccept.DLSettings.Bits.RX2DataRate;
      ...
      Nvm.MacGroup2.MacParams.ReceiveDelay2 =
          Nvm.MacGroup2.MacParams.ReceiveDelay1 + 1000;
      ...
      // MLME handling
      if (LoRaMacConfirmQueueIsCmdActive(MLME_JOIN) == true) {
        LoRaMacConfirmQueueSetStatus(LORAMAC_EVENT_INFO_STATUS_JOIN_FAIL,
                                     MLME_JOIN);
      }
    }
    break;
  ...
  case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
    ...
    phyParam = RegionGetPhyParam(Nvm.MacGroup2.Region, &getPhy);
    if ((MAX(0, (int16_t)((int16_t)size -
                          (int16_t)LORAMAC_FRAME_PAYLOAD_OVERHEAD_SIZE)) >
         (int16_t)phyParam.Value) ||
    ...
    for (uint8_t i = 0; i < LORAMAC_MAX_MC_CTX; i++) {
      if ((Nvm.MacGroup2.MulticastChannelList[i].ChannelParams.Address ==
           macMsgData.FHDR.DevAddr) &&
    ...
    if ((multicast == 1) &&
        ((fType != FRAME_TYPE_D) || (macMsgData.FHDR.FCtrl.Bits.Ack != 0) ||
    ...
    macCryptoStatus =
        GetFCntDown(addrID, fType, &macMsgData, Nvm.MacGroup2.Version, &fCntID,
                    &downLinkCounter);
    ...
    macCryptoStatus = LoRaMacCryptoUnsecureMessage(
        addrID, address, fCntID, downLinkCounter, &macMsgData);
    if (macCryptoStatus != LORAMAC_CRYPTO_SUCCESS) {
    ...
    if (multicast == 1) {
      ...
      if (macHdr.Bits.MType == FRAME_TYPE_DATA_CONFIRMED_DOWN) {
        ...
        if ((MacCtx.McpsIndication.RxSlot != RX_SLOT_WIN_1) &&
            (MacCtx.McpsIndication.RxSlot != RX_SLOT_WIN_2)) {
          // Calculate timeout
          MacCtx.McpsIndication.ResponseTimeout =
              REGION_COMMON_CLASS_B_C_RESP_TIMEOUT;
    ...
    RemoveMacCommands(MacCtx.McpsIndication.RxSlot, macMsgData.FHDR.FCtrl,
                      MacCtx.McpsConfirm.McpsRequest);
    ...
    case FRAME_TYPE_A: { /* +----------+------+-------+--------------+
      ...
      ProcessMacCommands(macMsgData.FHDR.FOpts, 0,
                         macMsgData.FHDR.FCtrl.Bits.FOptsLen, snr,
    ...
    case FRAME_TYPE_C: { /* +----------+------+-------+--------------+
      ...
      ProcessMacCommands(macMsgData.FRMPayload, 0, macMsgData.FRMPayloadSize,
                         snr, MacCtx.McpsIndication.RxSlot);
      MacCtx.McpsIndication.Port = macMsgData.FPort;
      break;
    }
    case FRAME_TYPE_D: { /* +----------+------+-------+--------------+
    ...
    if ((macMsgData.FPort == LORAMAC_CERT_FPORT) &&
        (Nvm.MacGroup2.IsCertPortOn ==
         false)) { // Do not notify the upper layer of data reception on FPort
                   // LORAMAC_CERT_FPORT if the port handling is disabled.
      MacCtx.McpsIndication.Port = macMsgData.FPort;
      MacCtx.McpsIndication.Buffer = NULL;
      MacCtx.McpsIndication.BufferSize = 0;
      MacCtx.McpsIndication.RxData = false;
...
static void
HandleRadioRxErrorTimeout(LoRaMacEventInfoStatus_t rx1EventInfoStatus,
  ...
  if (classBRx == false) {
    if (MacCtx.RxSlot == RX_SLOT_WIN_1) {
      ...
      if (TimerGetElapsedTime(Nvm.MacGroup1.LastTxDoneTime) >=
...
static void ProcessRadioRxError(void) {
  HandleRadioRxErrorTimeout(LORAMAC_EVENT_INFO_STATUS_RX1_ERROR,
                            LORAMAC_EVENT_INFO_STATUS_RX2_ERROR);
}

static void ProcessRadioRxTimeout(void) {
  HandleRadioRxErrorTimeout(LORAMAC_EVENT_INFO_STATUS_RX1_TIMEOUT,
    ...
    if (stopRetransmission == true) { // Stop retransmission
      ...
      if (IsReJoin0Required() == true) {
        SendReJoinReq(REJOIN_REQ_0);
               ...
               false) { // Arrange further retransmission
      MacCtx.MacFlags.Bits.MacDone = 0;
...
static void LoRaMacHandleMlmeRequest(void) {
  ...
  if (MacCtx.MacFlags.Bits.MlmeReq == 1) {
    if (LoRaMacConfirmQueueIsCmdActive(MLME_JOIN) == true) {
      if (LoRaMacConfirmQueueGetStatus(MLME_JOIN) ==
          LORAMAC_EVENT_INFO_STATUS_OK) { // Node joined successfully
...
static bool CheckForMinimumAbpDatarate(bool adr, ActivationType_t activation,
                                       bool datarateChanged) {
  if ((adr == true) && (activation == ACTIVATION_TYPE_ABP) &&
...
static void LoRaMacHandleNvm(LoRaMacNvmData_t *nvmData) {
  ...
  // Crypto
  crc = Crc32((uint8_t *)&nvmData->Crypto,
              sizeof(nvmData->Crypto) - sizeof(nvmData->Crypto.Crc32));
  ...
  // MacGroup1
  crc = Crc32((uint8_t *)&nvmData->MacGroup1,
              sizeof(nvmData->MacGroup1) - sizeof(nvmData->MacGroup1.Crc32));
  ...
  // MacGroup2
  crc = Crc32((uint8_t *)&nvmData->MacGroup2,
              sizeof(nvmData->MacGroup2) - sizeof(nvmData->MacGroup2.Crc32));
  ...
  // Secure Element
  crc = Crc32((uint8_t *)&nvmData->SecureElement,
  ...
  // Region
  crc = Crc32((uint8_t *)&nvmData->RegionGroup1,
              sizeof(nvmData->RegionGroup1) -
  ...
  crc = Crc32((uint8_t *)&nvmData->RegionGroup2,
              sizeof(nvmData->RegionGroup2) -
  ...
  // ClassB
  crc = Crc32((uint8_t *)&nvmData->ClassB,
              sizeof(nvmData->ClassB) - sizeof(nvmData->ClassB.Crc32));
...
static void OnRxWindow1TimerEvent(void *context) {
  ...
  MacCtx.RxWindow1Config.DownlinkDwellTime =
...
static void OnRxWindow2TimerEvent(void *context) {
  ...
  MacCtx.RxWindow2Config.Frequency =
      Nvm.MacGroup2.MacParams.Rx2Channel.Frequency;
  MacCtx.RxWindow2Config.DownlinkDwellTime =
      Nvm.MacGroup2.MacParams.DownlinkDwellTime;
  ...
  case CLASS_A: {
    ...
    if (deviceClass == CLASS_C) {
      ...
      for (int8_t i = 0; i < LORAMAC_MAX_MC_CTX; i++) {
        ...
        {
          Nvm.MacGroup2.MacParams.RxCChannel.Frequency =
              Nvm.MacGroup2.MulticastChannelList[i]
                  .ChannelParams.RxParams.ClassC.Frequency;
          Nvm.MacGroup2.MacParams.RxCChannel.Datarate =
              Nvm.MacGroup2.MulticastChannelList[i]
                  .ChannelParams.RxParams.ClassC.Datarate;

          MacCtx.RxWindowCConfig.Channel = MacCtx.Channel;
          MacCtx.RxWindowCConfig.Frequency =
              Nvm.MacGroup2.MacParams.RxCChannel.Frequency;
          MacCtx.RxWindowCConfig.DownlinkDwellTime =
              Nvm.MacGroup2.MacParams.DownlinkDwellTime;
      ...
      // Add a DeviceModeInd MAC Command to indicate the network a device mode
      // change.
      if (Nvm.MacGroup2.Version.Fields.Minor >= 1) {
        LoRaMacCommandsAddCmd(MOTE_MAC_DEVICE_MODE_IND,
                              (uint8_t *)&Nvm.MacGroup2.DeviceClass, 1);
      }
...
static void ProcessMacCommands(uint8_t *payload, uint8_t macIndex,
  ...
  while (macIndex < commandsSize) {
    ...
    if ((LoRaMacCommandsGetCmdSize(payload[macIndex]) + macIndex) >
        commandsSize) {
      return;
    }
    ...
    case SRV_MAC_RESET_CONF: {
      ...
      if (Nvm.MacGroup2.Version.Fields.Minor >= serverMinorVersion) {
        ...
        if (LoRaMacCommandsGetCmd(MOTE_MAC_RESET_IND, &macCmd) ==
    ...
    case SRV_MAC_LINK_CHECK_ANS: {
      if (LoRaMacConfirmQueueIsCmdActive(MLME_LINK_CHECK) == true) {
        LoRaMacConfirmQueueSetStatus(LORAMAC_EVENT_INFO_STATUS_OK,
                                     MLME_LINK_CHECK);
    ...
    case SRV_MAC_LINK_ADR_REQ: {
      ...
      if (adrBlockFound == false) {
        ...
        do {
          ...
          // LinkAdrAns contains the same value. This does not hold when ADR is
          // off, where every LinkAdrAns requires an individual status.
          if (Nvm.MacGroup2.AdrCtrlOn == true) {
            // When ADR is on, the function RegionLinkAdrReq will take care
            // about the parsing and interpretation of the LinkAdrRequest block
            // and it provides one status which shall be applied to every
            // LinkAdrAns
            linkAdrReq.PayloadSize = commandsSize - (macIndex - 1);
          } else {
            // When ADR is off, this function will loop over the individual
            // LinkAdrRequests and will call RegionLinkAdrReq for each
            // individually, as every request requires an individual answer.
            // When ADR is off, the function RegionLinkAdrReq ignores the new
            // values for ChannelsDatarate, ChannelsTxPower and ChannelsNbTrans.
            linkAdrReq.PayloadSize = 5;
          }

          // Process the ADR requests
          status = RegionLinkAdrReq(Nvm.MacGroup2.Region, &linkAdrReq,
    ...
    case SRV_MAC_DEV_STATUS_REQ: {
      ...
      if ((MacCtx.MacCallbacks != NULL) &&
          (MacCtx.MacCallbacks->GetBatteryLevel != NULL)) {
    ...
    case SRV_MAC_TX_PARAM_SETUP_REQ: {
      ...
      if (RegionTxParamSetupReq(Nvm.MacGroup2.Region, &txParamSetupReq) != -1) {
        // Accept command
        Nvm.MacGroup2.MacParams.UplinkDwellTime =
            txParamSetupReq.UplinkDwellTime;
        Nvm.MacGroup2.MacParams.DownlinkDwellTime =
            txParamSetupReq.DownlinkDwellTime;
        Nvm.MacGroup2.MacParams.MaxEirp =
            LoRaMacMaxEirpTable[txParamSetupReq.MaxEirp];
        // Update the datarate in case of the new configuration limits it
        getPhy.Attribute = PHY_MIN_TX_DR;
        getPhy.UplinkDwellTime = Nvm.MacGroup2.MacParams.UplinkDwellTime;
        phyParam = RegionGetPhyParam(Nvm.MacGroup2.Region, &getPhy);
        Nvm.MacGroup1.ChannelsDatarate =
            MAX(Nvm.MacGroup1.ChannelsDatarate, (int8_t)phyParam.Value);
    ...
    case SRV_MAC_REKEY_CONF: {
      ...
      if (Nvm.MacGroup2.Version.Fields.Minor >= serverMinorVersion) {
        ...
        if (LoRaMacCommandsGetCmd(MOTE_MAC_REKEY_IND, &macCmd) ==
            LORAMAC_COMMANDS_SUCCESS) {
          LoRaMacCommandsRemoveCmd(macCmd);
        }
      }
    ...
    case SRV_MAC_FORCE_REJOIN_REQ: {
      ...
      // TimerSetValue( &MacCtx.ForceRejoinReqCycleTimer,
      // forceRejoinReqCycleTime );

      if ((Nvm.MacGroup2.ForceRejoinType == 0) ||
          (Nvm.MacGroup2.ForceRejoinType == 1)) {
        SendReJoinReq(REJOIN_REQ_0);
      } else {
        SendReJoinReq(REJOIN_REQ_2);
      }
      TimerSetValue(&MacCtx.ForceRejoinReqCycleTimer, 0,
                    forceRejoinReqCycleTime);
    ...
    case SRV_MAC_REJOIN_PARAM_REQ: {
      ...
      if (ConvertRejoinCycleTime(Nvm.MacGroup2.Rejoin0CycleInSec,
                                 &MacCtx.Rejoin0CycleTime) == true) {
        timeOK = 0x01;
    ...
    case SRV_MAC_DEVICE_MODE_CONF: {
      ...
      macIndex++;
      if (LoRaMacCommandsGetCmd(MOTE_MAC_DEVICE_MODE_IND, &macCmd) ==
          LORAMAC_COMMANDS_SUCCESS) {
        LoRaMacCommandsRemoveCmd(macCmd);
      }
      break;
    ...
    case SRV_MAC_DEVICE_TIME_ANS: {
      ...
      if (LoRaMacConfirmQueueIsCmdActive(MLME_DEVICE_TIME) == true) {
        ...
        gpsEpochTime.SubSeconds =
            (int16_t)(((int32_t)gpsEpochTime.SubSeconds * 1000) >> 8);
        ...
        sysTime = SysTimeAdd(sysTimeCurrent,
                             SysTimeSub(sysTime, MacCtx.LastTxSysTime));
        ...
        // Incase of other receive windows the Device Time Answer is not
        // received.
        MacCtx.McpsIndication.DeviceTimeAnsReceived = false;
      }
      break;
    ...
    case SRV_MAC_PING_SLOT_INFO_ANS: {
      if (LoRaMacConfirmQueueIsCmdActive(MLME_PING_SLOT_INFO) == true) {
        LoRaMacConfirmQueueSetStatus(LORAMAC_EVENT_INFO_STATUS_OK,
                                     MLME_PING_SLOT_INFO);
        // According to the specification, it is not allowed to process this
        // answer in a ping or multicast slot
        if ((MacCtx.RxSlot != RX_SLOT_WIN_CLASS_B_PING_SLOT) &&
            (MacCtx.RxSlot != RX_SLOT_WIN_CLASS_B_MULTICAST_SLOT)) {
...
LoRaMacStatus_t Send(LoRaMacHeader_t *macHdr, uint8_t fPort, void *fBuffer,
  ...
  fCtrl.Bits.AdrAckReq = LoRaMacAdrCalcNext(
      &adrNext, &Nvm.MacGroup1.ChannelsDatarate, &Nvm.MacGroup1.ChannelsTxPower,
  ...
  // Validate status
  if ((status == LORAMAC_STATUS_OK) ||
      (status == LORAMAC_STATUS_SKIPPED_APP_DATA)) {
...
LoRaMacStatus_t SendReJoinReq(JoinReqIdentifier_t joinReqType) {
  ...
  case REJOIN_REQ_1: {
    ...
    memcpy1(MacCtx.TxMsg.Message.ReJoin1.JoinEUI, SecureElementGetJoinEui(),
            LORAMAC_JOIN_EUI_FIELD_SIZE);
    memcpy1(MacCtx.TxMsg.Message.ReJoin1.DevEUI, SecureElementGetDevEui(),
            LORAMAC_DEV_EUI_FIELD_SIZE);

    if (LORAMAC_CRYPTO_SUCCESS !=
        LoRaMacCryptoGetRJcount(RJ_COUNT_1,
  ...
  case REJOIN_REQ_2: {
    ...
    MacCtx.TxMsg.Message.ReJoin0or2.NetID[1] =
        (Nvm.MacGroup2.NetID >> 8) & 0xFF;
    MacCtx.TxMsg.Message.ReJoin0or2.NetID[2] =
        (Nvm.MacGroup2.NetID >> 16) & 0xFF;

    memcpy1(MacCtx.TxMsg.Message.ReJoin0or2.DevEUI, SecureElementGetDevEui(),
            LORAMAC_DEV_EUI_FIELD_SIZE);

    if (LORAMAC_CRYPTO_SUCCESS !=
        LoRaMacCryptoGetRJcount(RJ_COUNT_0,
                                &MacCtx.TxMsg.Message.ReJoin0or2.RJcount0)) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }
  ...
  case JOIN_REQ: {
    ...
    memcpy1(MacCtx.TxMsg.Message.JoinReq.JoinEUI, SecureElementGetJoinEui(),
            LORAMAC_JOIN_EUI_FIELD_SIZE);
...
static void ComputeRxWindowParameters(void) {
  ...
  RegionComputeRxWindowParameters(
      ...
      RegionApplyDrOffset(
          Nvm.MacGroup2.Region, Nvm.MacGroup2.MacParams.DownlinkDwellTime,
          Nvm.MacGroup1.ChannelsDatarate, Nvm.MacGroup2.MacParams.Rx1DrOffset),
      Nvm.MacGroup2.MacParams.MinRxSymbols,
      Nvm.MacGroup2.MacParams.SystemMaxRxError, &MacCtx.RxWindow1Config);
  // Compute Rx2 windows parameters
  RegionComputeRxWindowParameters(
      Nvm.MacGroup2.Region, Nvm.MacGroup2.MacParams.Rx2Channel.Datarate,
      Nvm.MacGroup2.MacParams.MinRxSymbols,
      Nvm.MacGroup2.MacParams.SystemMaxRxError, &MacCtx.RxWindow2Config);

  // Default setup, in case the device joined
  MacCtx.RxWindow1Delay = Nvm.MacGroup2.MacParams.ReceiveDelay1 +
                          MacCtx.RxWindow1Config.WindowOffset;
  MacCtx.RxWindow2Delay = Nvm.MacGroup2.MacParams.ReceiveDelay2 +
                          MacCtx.RxWindow2Config.WindowOffset;

  if (Nvm.MacGroup2.NetworkActivation == ACTIVATION_TYPE_NONE) {
    MacCtx.RxWindow1Delay = Nvm.MacGroup2.MacParams.JoinAcceptDelay1 +
                            MacCtx.RxWindow1Config.WindowOffset;
    MacCtx.RxWindow2Delay = Nvm.MacGroup2.MacParams.JoinAcceptDelay2 +
                            MacCtx.RxWindow2Config.WindowOffset;
  }
}
...
static LoRaMacStatus_t VerifyTxFrame(void) {
  ...
  if (Nvm.MacGroup2.NetworkActivation != ACTIVATION_TYPE_NONE) {
    if (LoRaMacCommandsGetSizeSerializedCmds(&macCmdsSize) !=
        LORAMAC_COMMANDS_SUCCESS) {
      return LORAMAC_STATUS_MAC_COMMAD_ERROR;
    }

    if (ValidatePayloadLength(MacCtx.AppDataSize,
                              Nvm.MacGroup1.ChannelsDatarate,
                              macCmdsSize) == false) {
...
static LoRaMacStatus_t SerializeTxFrame(void) {
  ...
  case LORAMAC_MSG_TYPE_JOIN_REQUEST:
    serializeStatus =
        LoRaMacSerializerJoinRequest(&MacCtx.TxMsg.Message.JoinReq);
...
static LoRaMacStatus_t ScheduleTx(bool allowDelayedTx) {
  ...
  nextChan.ElapsedTimeSinceStartUp =
      SysTimeSub(SysTimeGetMcuTime(), Nvm.MacGroup2.InitializationTime);
  ...
  // Select channel
  status = RegionNextChannel(Nvm.MacGroup2.Region, &nextChan, &MacCtx.Channel,
                             &MacCtx.DutyCycleWaitTime,
                             &Nvm.MacGroup1.AggregatedTimeOff);
...
static LoRaMacStatus_t SecureFrame(uint8_t txDr, uint8_t txCh) {
  ...
  case LORAMAC_MSG_TYPE_JOIN_REQUEST:
    macCryptoStatus =
        LoRaMacCryptoPrepareJoinRequest(&MacCtx.TxMsg.Message.JoinReq);
    if (LORAMAC_CRYPTO_SUCCESS != macCryptoStatus) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }
    MacCtx.PktBufferLen = MacCtx.TxMsg.Message.JoinReq.BufSize;
  ...
  case LORAMAC_MSG_TYPE_RE_JOIN_1:
    macCryptoStatus =
        LoRaMacCryptoPrepareReJoinType1(&MacCtx.TxMsg.Message.ReJoin1);
    if (LORAMAC_CRYPTO_SUCCESS != macCryptoStatus) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }
  ...
  case LORAMAC_MSG_TYPE_RE_JOIN_0_2:
    macCryptoStatus =
        LoRaMacCryptoPrepareReJoinType0or2(&MacCtx.TxMsg.Message.ReJoin0or2);
    if (LORAMAC_CRYPTO_SUCCESS != macCryptoStatus) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }
  ...
  case LORAMAC_MSG_TYPE_DATA:
    ...
    macCryptoStatus = LoRaMacCryptoSecureMessage(fCntUp, txDr, txCh,
                                                 &MacCtx.TxMsg.Message.Data);
    if (LORAMAC_CRYPTO_SUCCESS != macCryptoStatus) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }
    MacCtx.PktBufferLen = MacCtx.TxMsg.Message.Data.BufSize;
...
static void CalculateBackOff(void) {
  // Make sure that the calculation of the backoff time for the aggregated time
  // off will only be done in case the value is zero. It will be set to zero in
  // the function RegionNextChannel.
  if (Nvm.MacGroup1.AggregatedTimeOff == 0) {
    // Update aggregated time-off. This must be an assignment and no incremental
    // update as we do only calculate the time-off based on the last
    // transmission
    Nvm.MacGroup1.AggregatedTimeOff =
        (MacCtx.TxTimeOnAir * Nvm.MacGroup2.AggregatedDCycle -
         MacCtx.TxTimeOnAir);
  }
}
...
static void RemoveMacCommands(LoRaMacRxSlot_t rxSlot, LoRaMacFrameCtrl_t fCtrl,
  ...
  if (rxSlot == RX_SLOT_WIN_1 || rxSlot == RX_SLOT_WIN_2) {
    ...
    if (request == MCPS_CONFIRMED) {
      if (fCtrl.Bits.Ack ==
          1) { // For confirmed uplinks only if we have received an ACK.
...
static void ResetMacParameters(void) {
  ...
  Nvm.MacGroup2.MacParams.Rx1DrOffset =
      Nvm.MacGroup2.MacParamsDefaults.Rx1DrOffset;
  Nvm.MacGroup2.MacParams.Rx2Channel =
      Nvm.MacGroup2.MacParamsDefaults.Rx2Channel;
  Nvm.MacGroup2.MacParams.RxCChannel =
      Nvm.MacGroup2.MacParamsDefaults.RxCChannel;
  Nvm.MacGroup2.MacParams.UplinkDwellTime =
      Nvm.MacGroup2.MacParamsDefaults.UplinkDwellTime;
  Nvm.MacGroup2.MacParams.DownlinkDwellTime =
      Nvm.MacGroup2.MacParamsDefaults.DownlinkDwellTime;
  Nvm.MacGroup2.MacParams.MaxEirp = Nvm.MacGroup2.MacParamsDefaults.MaxEirp;
  Nvm.MacGroup2.MacParams.AntennaGain =
      Nvm.MacGroup2.MacParamsDefaults.AntennaGain;
  ...
  if (MacCtx.MacCallbacks != NULL) {
    classBCallbacks.GetTemperatureLevel =
        MacCtx.MacCallbacks->GetTemperatureLevel;
...
static bool IsReJoin0Required() {
  ...
  if ((Nvm.MacGroup2.Rejoin0UplinksLimit ==
...
static void RxWindowSetup(TimerEvent_t *rxTimer, RxConfigParams_t *rxConfig) {
  ...
  if (RegionRxConfig(Nvm.MacGroup2.Region, rxConfig,
                     (int8_t *)&MacCtx.McpsIndication.RxDatarate) == true) {
...
static void OpenContinuousRxCWindow(void) {
  ...
  RegionComputeRxWindowParameters(
      Nvm.MacGroup2.Region, Nvm.MacGroup2.MacParams.RxCChannel.Datarate,
      Nvm.MacGroup2.MacParams.MinRxSymbols,
      Nvm.MacGroup2.MacParams.SystemMaxRxError, &MacCtx.RxWindowCConfig);
  ...
  if (RegionRxConfig(Nvm.MacGroup2.Region, &MacCtx.RxWindowCConfig,
                     (int8_t *)&MacCtx.McpsIndication.RxDatarate) == true) {
...
LoRaMacStatus_t PrepareFrame(LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl,
  ...
  case FRAME_TYPE_DATA_UNCONFIRMED_UP:
    ...
    if (macCmdsSize > 0) {
      availableSize =
          GetMaxAppPayloadWithoutFOptsLength(Nvm.MacGroup1.ChannelsDatarate);

      // There is application payload available and the MAC commands fit into
      // FOpts field.
      if ((MacCtx.AppDataSize > 0) &&
          (macCmdsSize <= LORA_MAC_COMMAND_MAX_FOPTS_LENGTH)) {
        if (LoRaMacCommandsSerializeCmds(
                LORA_MAC_COMMAND_MAX_FOPTS_LENGTH, &macCmdsSize,
                MacCtx.TxMsg.Message.Data.FHDR.FOpts) !=
            LORAMAC_COMMANDS_SUCCESS) {
          return LORAMAC_STATUS_MAC_COMMAD_ERROR;
        }
      ...
      // There is application payload available but the MAC commands does NOT
      // fit into FOpts field.
      else if ((MacCtx.AppDataSize > 0) &&
               (macCmdsSize > LORA_MAC_COMMAND_MAX_FOPTS_LENGTH)) {
      ...
      // No application payload available therefore add all mac commands to the
      // FRMPayload.
      else {
        if (LoRaMacCommandsSerializeCmds(availableSize, &macCmdsSize,
                                         MacCtx.MacCommandsBuffer) !=
  ...
  case FRAME_TYPE_PROPRIETARY:
    if ((fBuffer != NULL) && (MacCtx.AppDataSize > 0)) {
      memcpy1(MacCtx.PktBuffer + LORAMAC_MHDR_FIELD_SIZE, (uint8_t *)fBuffer,
              MacCtx.AppDataSize);
...
LoRaMacStatus_t SendFrameOnChannel(uint8_t channel) {
  ...
  RegionTxConfig(Nvm.MacGroup2.Region, &txConfig, &txPower,
                 &MacCtx.TxTimeOnAir);
  ...
  if (LoRaMacClassBIsBeaconModeActive() == true) {
    // Currently, the Time-On-Air can only be computed when the radio is
    // configured with the TX configuration
    TimerTime_t collisionTime =
        LoRaMacClassBIsUplinkCollision(MacCtx.TxTimeOnAir);
...
LoRaMacNvmData_t *GetNvmData(void) { return &Nvm; }
...
LoRaMacStatus_t RestoreNvmData(LoRaMacNvmData_t *nvm) {
  ...
  // Crypto
  crc = Crc32((uint8_t *)&nvm->Crypto,
              sizeof(nvm->Crypto) - sizeof(nvm->Crypto.Crc32));
  ...
  // MacGroup1
  crc = Crc32((uint8_t *)&nvm->MacGroup1,
              sizeof(nvm->MacGroup1) - sizeof(nvm->MacGroup1.Crc32));
  ...
  // MacGroup2
  crc = Crc32((uint8_t *)&nvm->MacGroup2,
              sizeof(nvm->MacGroup2) - sizeof(nvm->MacGroup2.Crc32));
  ...
  // Region
  crc = Crc32((uint8_t *)&nvm->RegionGroup1,
              sizeof(nvm->RegionGroup1) - sizeof(nvm->RegionGroup1.Crc32));
  if (crc == nvm->RegionGroup1.Crc32) {
    memcpy1((uint8_t *)&Nvm.RegionGroup1, (uint8_t *)&nvm->RegionGroup1,
            sizeof(Nvm.RegionGroup1));
  }

  crc = Crc32((uint8_t *)&nvm->ClassB,
              sizeof(nvm->ClassB) - sizeof(nvm->ClassB.Crc32));
...
LoRaMacStatus_t DetermineFrameType(LoRaMacMessageData_t *macMsg,
  ...
  /* The LoRaWAN specification allows several possible configurations how data
   * up/down frames are built up. In sake of clearness the following naming is
   * applied. Please keep in mind that this is implementation specific since
   * there is no definition in the LoRaWAN specification included.
   *
...
static bool StopRetransmission(void) {
  ...
  if ((MacCtx.MacFlags.Bits.McpsInd == 0) ||
      ((MacCtx.McpsIndication.RxSlot != RX_SLOT_WIN_1) &&
       (MacCtx.McpsIndication.RxSlot !=
        RX_SLOT_WIN_2))) { // Maximum repetitions without downlink. Increase ADR
                           // Ack counter.
    // Only process the case when the MAC did not receive a downlink.
    if (Nvm.MacGroup2.AdrCtrlOn == true) {
      Nvm.MacGroup1.AdrAckCounter =
          IncreaseAdrAckCounter(Nvm.MacGroup1.AdrAckCounter);
    }
  }
...
LoRaMacStatus_t LoRaMacInitialization(LoRaMacPrimitives_t *primitives,
  ...
  // Verify if the region is supported 验证输入配置的区域信息是否支持
  ...
  Nvm.MacGroup2.MacParams.SystemMaxRxError =
      Nvm.MacGroup2.MacParamsDefaults.SystemMaxRxError;
  Nvm.MacGroup2.MacParams.MinRxSymbols =
      Nvm.MacGroup2.MacParamsDefaults.MinRxSymbols;
  Nvm.MacGroup2.MacParams.MaxRxWindow =
      Nvm.MacGroup2.MacParamsDefaults.MaxRxWindow;
  Nvm.MacGroup2.MacParams.ReceiveDelay1 =
      Nvm.MacGroup2.MacParamsDefaults.ReceiveDelay1;
  Nvm.MacGroup2.MacParams.ReceiveDelay2 =
      Nvm.MacGroup2.MacParamsDefaults.ReceiveDelay2;
  Nvm.MacGroup2.MacParams.JoinAcceptDelay1 =
      Nvm.MacGroup2.MacParamsDefaults.JoinAcceptDelay1;
  Nvm.MacGroup2.MacParams.JoinAcceptDelay2 =
      Nvm.MacGroup2.MacParamsDefaults.JoinAcceptDelay2;
  Nvm.MacGroup2.MacParams.ChannelsNbTrans =
      Nvm.MacGroup2.MacParamsDefaults.ChannelsNbTrans;
  ...
  // Initialize timers
  TimerInit(&MacCtx.TxDelayedTimer, OnTxDelayedTimerEvent, NULL);
  TimerInit(&MacCtx.RxWindowTimer1, OnRxWindow1TimerEvent, NULL);
  TimerInit(&MacCtx.RxWindowTimer2, OnRxWindow2TimerEvent, NULL);
  TimerInit(&MacCtx.RetransmitTimeoutTimer, OnRetransmitTimeoutTimerEvent,
            NULL);
  TimerInit(&MacCtx.Rejoin0CycleTimer, OnRejoin0CycleTimerEvent, NULL);
  TimerInit(&MacCtx.Rejoin1CycleTimer, OnRejoin1CycleTimerEvent, NULL);
  TimerInit(&MacCtx.Rejoin2CycleTimer, OnRejoin2CycleTimerEvent, NULL);
...
LoRaMacStatus_t LoRaMacQueryTxPossible(uint8_t size, LoRaMacTxInfo_t *txInfo) {
  ...
  txInfo->CurrentPossiblePayloadSize =
      GetMaxAppPayloadWithoutFOptsLength(datarate);
  ...
  if ((LORA_MAC_COMMAND_MAX_FOPTS_LENGTH >= macCmdsSize) &&
      (txInfo->CurrentPossiblePayloadSize >= macCmdsSize)) {
    txInfo->MaxPossibleApplicationDataSize =
        txInfo->CurrentPossiblePayloadSize - macCmdsSize;

    // Verify if the application data together with MAC command fit into the
    // maximum payload.
...
LoRaMacStatus_t LoRaMacMibGetRequestConfirm(MibRequestConfirm_t *mibGet) {
  ...
  case MIB_CHANNELS_DEFAULT_DATARATE: {
    mibGet->Param.ChannelsDefaultDatarate =
        Nvm.MacGroup2.ChannelsDatarateDefault;
    break;
  }
  ...
  case MIB_DEFAULT_ANTENNA_GAIN: {
    mibGet->Param.DefaultAntennaGain =
        Nvm.MacGroup2.MacParamsDefaults.AntennaGain;
    break;
  }
...
LoRaMacStatus_t LoRaMacMibSetRequestConfirm(MibRequestConfirm_t *mibSet) {
  ...
  case MIB_NETWORK_ACTIVATION: {
    ...
    } else { // Do not allow to set ACTIVATION_TYPE_OTAA since the MAC will set
             // it automatically after a successful join process.
      status = LORAMAC_STATUS_PARAMETER_INVALID;
    }
    break;
  }
  case MIB_DEV_EUI: {
    if (SecureElementSetDevEui(mibSet->Param.DevEui) !=
  ...
  case MIB_JOIN_EUI: {
    if (SecureElementSetJoinEui(mibSet->Param.JoinEui) !=
        SECURE_ELEMENT_SUCCESS) {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
    }
    break;
  ...
  case MIB_APP_KEY: {
    if (mibSet->Param.AppKey != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(APP_KEY, mibSet->Param.AppKey)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_NWK_KEY: {
    if (mibSet->Param.NwkKey != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(NWK_KEY, mibSet->Param.NwkKey)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_J_S_INT_KEY: {
    if (mibSet->Param.JSIntKey != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(J_S_INT_KEY, mibSet->Param.JSIntKey)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_J_S_ENC_KEY: {
    if (mibSet->Param.JSEncKey != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(J_S_ENC_KEY, mibSet->Param.JSEncKey)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_F_NWK_S_INT_KEY: {
    if (mibSet->Param.FNwkSIntKey != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(F_NWK_S_INT_KEY, mibSet->Param.FNwkSIntKey)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_S_NWK_S_INT_KEY: {
    if (mibSet->Param.SNwkSIntKey != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(S_NWK_S_INT_KEY, mibSet->Param.SNwkSIntKey)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_NWK_S_ENC_KEY: {
    if (mibSet->Param.NwkSEncKey != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(NWK_S_ENC_KEY, mibSet->Param.NwkSEncKey)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_APP_S_KEY: {
    if (mibSet->Param.AppSKey != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(APP_S_KEY, mibSet->Param.AppSKey)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_KE_KEY: {
    if (mibSet->Param.McKEKey != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_KE_KEY, mibSet->Param.McKEKey)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_KEY_0: {
    if (mibSet->Param.McKey0 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_KEY_0, mibSet->Param.McKey0)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_APP_S_KEY_0: {
    if (mibSet->Param.McAppSKey0 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_APP_S_KEY_0, mibSet->Param.McAppSKey0)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_NWK_S_KEY_0: {
    if (mibSet->Param.McNwkSKey0 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_NWK_S_KEY_0, mibSet->Param.McNwkSKey0)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_KEY_1: {
    if (mibSet->Param.McKey1 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_KEY_1, mibSet->Param.McKey1)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_APP_S_KEY_1: {
    if (mibSet->Param.McAppSKey1 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_APP_S_KEY_1, mibSet->Param.McAppSKey1)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_NWK_S_KEY_1: {
    if (mibSet->Param.McNwkSKey1 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_NWK_S_KEY_1, mibSet->Param.McNwkSKey1)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_KEY_2: {
    if (mibSet->Param.McKey2 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_KEY_2, mibSet->Param.McKey2)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_APP_S_KEY_2: {
    if (mibSet->Param.McAppSKey2 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_APP_S_KEY_2, mibSet->Param.McAppSKey2)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_NWK_S_KEY_2: {
    if (mibSet->Param.McNwkSKey2 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_NWK_S_KEY_2, mibSet->Param.McNwkSKey2)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_KEY_3: {
    if (mibSet->Param.McKey3 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_KEY_3, mibSet->Param.McKey3)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_APP_S_KEY_3: {
    if (mibSet->Param.McAppSKey3 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_APP_S_KEY_3, mibSet->Param.McAppSKey3)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_MC_NWK_S_KEY_3: {
    if (mibSet->Param.McNwkSKey3 != NULL) {
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetKey(MC_NWK_S_KEY_3, mibSet->Param.McNwkSKey3)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_RXC_CHANNEL: {
    ...
    if (RegionVerify(Nvm.MacGroup2.Region, &verify, PHY_RX_DR) == true) {
      ...
      if ((Nvm.MacGroup2.DeviceClass == CLASS_C) &&
          (Nvm.MacGroup2.NetworkActivation != ACTIVATION_TYPE_NONE)) {
        // We can only compute the RX window parameters directly, if we are
        // already in class c mode and joined. We cannot setup an RX window in
        // case of any other class type. Set the radio into sleep mode in case
        // we are still in RX mode
        Radio.Sleep();
  ...
  case MIB_RXC_DEFAULT_CHANNEL: {
    ...
    if (RegionVerify(Nvm.MacGroup2.Region, &verify, PHY_RX_DR) == true) {
      Nvm.MacGroup2.MacParamsDefaults.RxCChannel =
          mibSet->Param.RxCDefaultChannel;
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
    }
    break;
  ...
  case MIB_DEFAULT_ANTENNA_GAIN: {
    Nvm.MacGroup2.MacParamsDefaults.AntennaGain =
        mibSet->Param.DefaultAntennaGain;
    break;
  }
  case MIB_NVM_CTXS: {
  ...
  case MIB_ABP_LORAWAN_VERSION: {
    if (mibSet->Param.AbpLrWanVersion.Fields.Minor <= 1) {
      ...
      if (LORAMAC_CRYPTO_SUCCESS !=
          LoRaMacCryptoSetLrWanVersion(mibSet->Param.AbpLrWanVersion)) {
        return LORAMAC_STATUS_CRYPTO_ERROR;
      }
    } else {
      status = LORAMAC_STATUS_PARAMETER_INVALID;
  ...
  case MIB_REJOIN_1_CYCLE: {
    if (ConvertRejoinCycleTime(Nvm.MacGroup2.Rejoin1CycleInSec,
                               &MacCtx.Rejoin1CycleTime) == true) {
...
LoRaMacStatus_t LoRaMacMcChannelSetup(McChannelParams_t *channel) {
  ...
  if (channel->IsRemotelySetup == true) {
    const KeyIdentifier_t mcKeys[LORAMAC_MAX_MC_CTX] = {MC_KEY_0, MC_KEY_1,
                                                        MC_KEY_2, MC_KEY_3};
    if (LoRaMacCryptoSetKey(mcKeys[channel->GroupID], channel->McKeys.McKeyE) !=
        LORAMAC_CRYPTO_SUCCESS) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }

    if (LoRaMacCryptoDeriveMcSessionKeyPair(
            channel->GroupID, channel->Address) != LORAMAC_CRYPTO_SUCCESS) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }
  } else {
    ...
    if (LORAMAC_CRYPTO_SUCCESS !=
        LoRaMacCryptoSetKey(mcAppSKeys[channel->GroupID],
                            channel->McKeys.Session.McAppSKey)) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }
    if (LORAMAC_CRYPTO_SUCCESS !=
        LoRaMacCryptoSetKey(mcNwkSKeys[channel->GroupID],
                            channel->McKeys.Session.McNwkSKey)) {
      return LORAMAC_STATUS_CRYPTO_ERROR;
    }
  }
  ...
  if (channel->Class == CLASS_B) {
    ...
    LoRaMacClassBSetMulticastPeriodicity(
        &Nvm.MacGroup2.MulticastChannelList[channel->GroupID]);
  }

  // Reset multicast channel downlink counter to initial value.
  *Nvm.MacGroup2.MulticastChannelList[channel->GroupID].DownLinkCounter =
...
LoRaMacStatus_t LoRaMacMcChannelSetupRxParams(AddressIdentifier_t groupID,
  ...
  *status = 0x1C + (groupID & 0x03);
  ...
  DeviceClass_t devClass =
      Nvm.MacGroup2.MulticastChannelList[groupID].ChannelParams.Class;
  ...
  *status &= 0x0F; // groupID OK
  ...
  if (*status == (groupID & 0x03)) {
    // Apply parameters
    Nvm.MacGroup2.MulticastChannelList[groupID].ChannelParams.RxParams =
        *rxParams;
  }
  return LORAMAC_STATUS_OK;
}
...
LoRaMacStatus_t LoRaMacMlmeRequest(MlmeReq_t *mlmeRequest) {
  ...
  case MLME_JOIN: {
    ...
    if (mlmeRequest->Req.Join.NetworkActivation == ACTIVATION_TYPE_OTAA) {
      ...
      if (status != LORAMAC_STATUS_OK) {
        // Revert back the previous datarate ( mainly used for US915 like
        // regions )
        Nvm.MacGroup1.ChannelsDatarate = RegionAlternateDr(
  ...
  case MLME_LINK_CHECK: {
    ...
    status = LORAMAC_STATUS_OK;
    if (LoRaMacCommandsAddCmd(MOTE_MAC_LINK_CHECK_REQ, macCmdPayload, 0) !=
        LORAMAC_COMMANDS_SUCCESS) {
  ...
  case MLME_DEVICE_TIME: {
    // LoRaMac will send this command piggy-pack
    status = LORAMAC_STATUS_OK;
    if (LoRaMacCommandsAddCmd(MOTE_MAC_DEVICE_TIME_REQ, macCmdPayload, 0) !=
        LORAMAC_COMMANDS_SUCCESS) {
      status = LORAMAC_STATUS_MAC_COMMAD_ERROR;
    }
    break;
  ...
  case MLME_PING_SLOT_INFO: {
    if (Nvm.MacGroup2.DeviceClass == CLASS_A) {
      ...
      LoRaMacClassBSetPingSlotInfo(
          mlmeRequest->Req.PingSlotInfo.PingSlot.Fields.Periodicity);
      macCmdPayload[0] = value;
      status = LORAMAC_STATUS_OK;
      if (LoRaMacCommandsAddCmd(MOTE_MAC_PING_SLOT_INFO_REQ, macCmdPayload,
                                1) != LORAMAC_COMMANDS_SUCCESS) {
        status = LORAMAC_STATUS_MAC_COMMAD_ERROR;
      }
    }
    break;
  ...
  case MLME_BEACON_TIMING: {
    // LoRaMac will send this command piggy-pack
    status = LORAMAC_STATUS_OK;
    if (LoRaMacCommandsAddCmd(MOTE_MAC_BEACON_TIMING_REQ, macCmdPayload, 0) !=
        LORAMAC_COMMANDS_SUCCESS) {
      status = LORAMAC_STATUS_MAC_COMMAD_ERROR;
    }
    break;
...
LoRaMacStatus_t LoRaMacMcpsRequest(McpsReq_t *mcpsRequest) {
  ...
  if (CheckForMinimumAbpDatarate(
          Nvm.MacGroup2.AdrCtrlOn, Nvm.MacGroup2.NetworkActivation,
          Nvm.MacGroup2.ChannelsDatarateChangedLinkAdrReq) == true) {
...
static void OnRejoin0CycleTimerEvent(void *context) {
  TimerStop(&MacCtx.Rejoin0CycleTimer);
  ConvertRejoinCycleTime(Nvm.MacGroup2.Rejoin0CycleInSec,
                         &MacCtx.Rejoin0CycleTime);
...
static void OnRejoin1CycleTimerEvent(void *context) {
  ...
  ConvertRejoinCycleTime(Nvm.MacGroup2.Rejoin1CycleInSec,
...
static void OnRejoin2CycleTimerEvent(void *context) {
  ...
  ConvertRejoinCycleTime(Nvm.MacGroup2.Rejoin2CycleInSec,
                         &MacCtx.Rejoin2CycleTime);
...
static void OnForceRejoinReqCycleTimerEvent(void *context) {
  if (Nvm.MacGroup1.ForceRejoinRetriesCounter ==

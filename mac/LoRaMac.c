...
static void ProcessRadioTxDone( void )
{
    ...
    ts_diff_tv(&now_ts, &Nvm.MacGroup2.InitializationTime, &txDone.ElapsedTimeSinceStartUp);
...
static LoRaMacStatus_t ScheduleTx( bool allowDelayedTx )
{
    ...
    struct timespec now_ts = {0};
    clock_gettime(CLOCK_MONOTONIC, &now_ts);
    ts_diff_tv(&now_ts, &Nvm.MacGroup2.InitializationTime, &nextChan.ElapsedTimeSinceStartUp);
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t* primitives, LoRaMacCallback_t* callbacks, LoRaMacRegion_t region )
{
    ...
    TimerInit( &MacCtx.RetransmitTimeoutTimer, OnRetransmitTimeoutTimerEvent , NULL);
    ...
    TimerInit( &MacCtx.ForceRejoinReqCycleTimer, OnForceRejoinReqCycleTimerEvent , NULL);

    // Store the current initialization time
    clock_gettime(CLOCK_MONOTONIC, &Nvm.MacGroup2.InitializationTime); //Nvm.MacGroup2.InitializationTime = SysTimeGetMcuTime( );

 ...
 */
static void ComputePingOffset(uint64_t beaconTime, uint32_t address,
                              uint16_t pingPeriod, uint16_t *pingOffset) {
  uint8_t buffer[16];
  uint8_t cipher[16];
  uint32_t result = 0;
 ...
 */
static uint32_t CalcDownlinkChannelAndFrequency(uint32_t devAddr,
  ...
  // nbChannels is > 1, when the channel plan requires more than one possible
  // channel defined by the calculation below.
  if (nbChannels > 1) {
    getPhy.Attribute = PHY_BEACON_CHANNEL_OFFSET;
    phyParam =
        RegionGetPhyParam(*Ctx.LoRaMacClassBParams.LoRaMacRegion, &getPhy);
...
/*!
 * \brief Calculates the correct frequency and opens up the beacon reception
 * window. Please note that the variable WindowTimeout and WindowOffset will be
 * updated according to the current settings. Also, the function perform a
 * calculation only, when Ctx.BeaconCtx.Ctrl.BeaconAcquired OR
 * Ctx.BeaconCtx.Ctrl.AcquisitionPending is set to 1.
 *
 * \param [IN] rxConfig Reception parameters for the beacon window.
 *
 * \param [IN] currentSymbolTimeout Current symbol timeout.
 */
static void CalculateBeaconRxWindowConfig(RxConfigParams_t *rxConfig,
                                          uint16_t currentSymbolTimeout) {
  GetPhyParams_t getPhy;
  PhyParam_t phyParam;
  ...
  if ((Ctx.BeaconCtx.Ctrl.BeaconAcquired == 1) ||
    ...
    // Compare and assign the maximum between the region specific rx error
    // window time and time precision received from beacon frame format.
    ...
    RegionComputeRxWindowParameters(
        *Ctx.LoRaMacClassBParams.LoRaMacRegion,
        (int8_t)phyParam.Value, // datarate
        Ctx.LoRaMacClassBParams.LoRaMacParams->MinRxSymbols, maxRxError,
        rxConfig);
  }
}
 ...
 *
 * \param [IN] activateDefaultChannel Set to true, if the function shall setup
 * the default channel
 *
...
static void RxBeaconSetup(TimerTime_t rxTime, bool activateDefaultChannel,
  ...
  if (activateDefaultChannel == true) {
    ...
    frequency = CalcDownlinkChannelAndFrequency(
        0, Ctx.BeaconCtx.BeaconTime.Seconds + (CLASSB_BEACON_INTERVAL / 1000),
  ...
  RegionRxBeaconSetup(*Ctx.LoRaMacClassBParams.LoRaMacRegion, &rxBeaconSetup,
                      &Ctx.LoRaMacClassBParams.McpsIndication->RxDatarate);

  Ctx.LoRaMacClassBParams.MlmeIndication->BeaconInfo.Frequency = frequency;
  Ctx.LoRaMacClassBParams.MlmeIndication->BeaconInfo.Datarate =
      Ctx.LoRaMacClassBParams.McpsIndication->RxDatarate;
}
 ...
 */
static bool CalcNextSlotTime(uint16_t slotOffset, uint16_t pingPeriod,
                             uint16_t pingNb, TimerTime_t *timeOffset) {
  ...
  slotTime = ((currentTime - SysTimeToMs(Ctx.BeaconCtx.LastBeaconRx)) %
  ...
  if (slotTime < currentTime) {
    currentPingSlot =
        ((currentTime - slotTime) / (pingPeriod * CLASSB_PING_SLOT_WINDOW)) + 1;
    slotTime +=
        ((TimerTime_t)(currentPingSlot * pingPeriod) * CLASSB_PING_SLOT_WINDOW);
  }

  if (currentPingSlot < pingNb) {
    if (slotTime <= (SysTimeToMs(Ctx.BeaconCtx.NextBeaconRx) -
                     CLASSB_BEACON_GUARD - CLASSB_PING_SLOT_WINDOW)) {
...
static void GetTemperatureLevel(LoRaMacClassBCallback_t *callbacks,
                                BeaconContext_t *beaconCtx) {
...
static TimerTime_t CalcDelayForNextBeacon(TimerTime_t currentTime,
...

{
  ...
  beaconEventTime = CalcDelayForNextBeacon(
      currentTime, SysTimeToMs(Ctx.BeaconCtx.LastBeaconRx));
  Ctx.BeaconCtx.NextBeaconRx = SysTimeFromMs(currentTime + beaconEventTime);

  // Take temperature compensation into account
  beaconEventTime =
      TimerTempCompensation(beaconEventTime, Ctx.BeaconCtx.Temperature);
...
static bool CheckSlotPriority(uint32_t currentAddress,
                              uint8_t currentFPendingSet,
                              uint8_t currentIsMulticast, uint32_t address,
                              uint8_t fPendingSet, uint8_t isMulticast) {
...
static void LoRaMacClassBProcessBeacon(void) {
  ...
  case BEACON_STATE_ACQUISITION_BY_TIME: {
    ...
    if (Ctx.BeaconCtx.Ctrl.AcquisitionPending == 1) {
      ...
      if (Ctx.BeaconCtx.Ctrl.BeaconDelaySet == 1) {
        ...
        CalculateBeaconRxWindowConfig(&beaconRxConfig,
        ...
        if (Ctx.BeaconCtx.BeaconTimingDelay > 0) {
          if (SysTimeToMs(Ctx.BeaconCtx.NextBeaconRx) > currentTime) {
            ...
            beaconEventTime = TimerTempCompensation(
                SysTimeToMs(Ctx.BeaconCtx.NextBeaconRx) - currentTime,
                Ctx.BeaconCtx.Temperature);

            if ((int32_t)beaconEventTime > beaconRxConfig.WindowOffset) {
              // Apply the offset of the system error respectively beaconing
              // precision setting
  ...
  case BEACON_STATE_ACQUISITION: {
    ...
    if (Ctx.BeaconCtx.Ctrl.AcquisitionPending == 1) {
      ...
      // Start the beacon acquisition. When the MAC has received a beacon in
      // function RxBeacon successfully, the next state is BEACON_STATE_LOCKED.
      // If the MAC does not find a beacon, the state machine will stay in state
      // BEACON_STATE_ACQUISITION. This state detects that a acquisition was
      // pending previously and will change the next state to BEACON_STATE_LOST.
  ...
  case BEACON_STATE_REACQUISITION: {
    ...
    if ((currentTime - SysTimeToMs(Ctx.BeaconCtx.LastBeaconRx)) >
        CLASSB_MAX_BEACON_LESS_PERIOD) {
      Ctx.BeaconState = BEACON_STATE_LOST;
    } else {
      // Handle beacon miss
      beaconEventTime =
          UpdateBeaconState(LORAMAC_EVENT_INFO_STATUS_BEACON_LOST,
  ...
  case BEACON_STATE_IDLE: {
    ...
    GetTemperatureLevel(&Ctx.LoRaMacClassBCallbacks, &Ctx.BeaconCtx);
    beaconEventTime =
        Ctx.BeaconCtx.NextBeaconRxAdjusted - Radio.GetWakeupTime();
    currentTime = TimerGetCurrentTime();

    // The goal is to calculate beaconRxConfig.WindowTimeout and
    // beaconRxConfig.WindowOffset
    ...
    if (beaconEventTime > currentTime) {
      ...
      if ((int32_t)beaconEventTime > beaconRxConfig.WindowOffset) {
        // Apply the offset of the system error respectively beaconing precision
        // setting
        beaconEventTime += beaconRxConfig.WindowOffset;
      }
    } else {
  ...
  case BEACON_STATE_LOST: {
    ...
    if (Ctx.LoRaMacClassBParams.LoRaMacFlags->Bits.MlmeReq == 1) {
      if (LoRaMacConfirmQueueIsCmdActive(MLME_BEACON_ACQUISITION) == true) {
        LoRaMacConfirmQueueSetStatus(LORAMAC_EVENT_INFO_STATUS_BEACON_NOT_FOUND,
                                     MLME_BEACON_ACQUISITION);
      }
    } else {
      Ctx.LoRaMacClassBParams.MlmeIndication->MlmeIndication = MLME_BEACON_LOST;
      Ctx.LoRaMacClassBParams.MlmeIndication->Status =
...
static void LoRaMacClassBProcessPingSlot(void) {
  ...
  case PINGSLOT_STATE_SET_TIMER: {
    if (CalcNextSlotTime(
            Ctx.PingSlotCtx.PingOffset, ClassBNvm->PingSlotCtx.PingPeriod,
            ClassBNvm->PingSlotCtx.PingNb, &pingSlotTime) == true) {
      if (Ctx.BeaconCtx.Ctrl.BeaconAcquired == 1) {
        // Compare and assign the maximum between the region specific rx error
        // window time and time precision received from beacon frame format.
        maxRxError =
            MAX(Ctx.LoRaMacClassBParams.LoRaMacParams->SystemMaxRxError,
                (uint32_t)Ctx.BeaconCtx.BeaconTimePrecision.SubSeconds);
        ...
        if ((int32_t)pingSlotTime >
            pingSlotRxConfig.WindowOffset) { // Apply the window offset
  ...
  case PINGSLOT_STATE_IDLE: {
    ...
    if (Ctx.PingSlotCtx.NextMulticastChannel != NULL) {
      ...
      slotHasPriority = CheckSlotPriority(
          *Ctx.LoRaMacClassBParams.LoRaMacDevAddr,
          ClassBNvm->PingSlotCtx.FPendingSet, 0,
    ...
    if ((Ctx.MulticastSlotState != PINGSLOT_STATE_RX) ||
        (slotHasPriority == true)) {
      if (Ctx.MulticastSlotState == PINGSLOT_STATE_RX) {
        // Close multicast slot window, if necessary. Multicast slots have
        // priority
        Radio.Standby();
        Ctx.MulticastSlotState = PINGSLOT_STATE_CALC_PING_OFFSET;
      ...
      pingSlotRxConfig.DownlinkDwellTime =
          Ctx.LoRaMacClassBParams.LoRaMacParams->DownlinkDwellTime;
      ...
      pingSlotRxConfig.NetworkActivation =
          *Ctx.LoRaMacClassBParams.NetworkActivation;

      RegionRxConfig(
          *Ctx.LoRaMacClassBParams.LoRaMacRegion, &pingSlotRxConfig,
          (int8_t *)&Ctx.LoRaMacClassBParams.McpsIndication->RxDatarate);
...
static void LoRaMacClassBProcessMulticastSlot(void) {
  ...
  case PINGSLOT_STATE_CALC_PING_OFFSET: {
    ...
    for (uint8_t i = 0; i < 4; i++) {
      ComputePingOffset(Ctx.BeaconCtx.BeaconTime.Seconds,
                        cur->ChannelParams.Address, cur->PingPeriod,
                        &(cur->PingOffset));
      cur++;
    }
  ...
  case PINGSLOT_STATE_SET_TIMER: {
    ...
    for (uint8_t i = 0; i < LORAMAC_MAX_MC_CTX; i++) {
      ...
      if (CalcNextSlotTime(cur->PingOffset, cur->PingPeriod, cur->PingNb,
                           &slotTime) == true) {
    ...
    if (Ctx.PingSlotCtx.NextMulticastChannel != NULL) {
      ...
      if ((int32_t)multicastSlotTime >
          multicastSlotRxConfig.WindowOffset) { // Apply the window offset
  ...
  case PINGSLOT_STATE_IDLE: {
    ...
    // Apply frequency
    frequency = Ctx.PingSlotCtx.NextMulticastChannel->ChannelParams.RxParams
                    .ClassB.Frequency;

    // Restore the floor plan frequency if there is no individual frequency
    // assigned
    if (frequency == 0) {
      // Restore floor plan
      frequency = CalcDownlinkChannelAndFrequency(
    ...
    if ((Ctx.PingSlotState != PINGSLOT_STATE_RX) || (slotHasPriority == true)) {
      ...
      multicastSlotRxConfig.Datarate =
          Ctx.PingSlotCtx.NextMulticastChannel->ChannelParams.RxParams.ClassB
              .Datarate;
      multicastSlotRxConfig.DownlinkDwellTime =
          Ctx.LoRaMacClassBParams.LoRaMacParams->DownlinkDwellTime;
      ...
      multicastSlotRxConfig.NetworkActivation =
          *Ctx.LoRaMacClassBParams.NetworkActivation;

      RegionRxConfig(
          *Ctx.LoRaMacClassBParams.LoRaMacRegion, &multicastSlotRxConfig,
          (int8_t *)&Ctx.LoRaMacClassBParams.McpsIndication->RxDatarate);
...
bool LoRaMacClassBRxBeacon(uint8_t *payload, uint16_t size) {
  ...
  if ((Ctx.BeaconState == BEACON_STATE_RX) ||
    ...
    if (size == phyParam.BeaconFormat.BeaconSize) {
      ...
      //
      // Field RFU1 and RFU2 have variable sizes. It depends on the region
      // specific implementation

      // Read CRC1 field from the frame
      beaconCrc0 =
          ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4]) & 0x00FF;
      beaconCrc0 |=
          ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4 + 1] << 8) &
          0xFF00;
      ...
      if (crc0 == beaconCrc0) {
        ...
        // Fetch the precise time value in milliseconds that will be used for Rx
        // ping slot delay.
        Ctx.BeaconCtx.BeaconTimePrecision.SubSeconds =
            BeaconPrecTimeValue[Ctx.LoRaMacClassBParams.MlmeIndication
                                    ->BeaconInfo.Param];

        // Read Time field from the frame
        Ctx.BeaconCtx.BeaconTime.Seconds =
            ((uint32_t)payload[phyParam.BeaconFormat.Rfu1Size + 1]) &
            0x000000FF;
        Ctx.BeaconCtx.BeaconTime.Seconds |=
            ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 2] << 8)) &
            0x0000FF00;
        Ctx.BeaconCtx.BeaconTime.Seconds |=
            ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 3] << 16)) &
            0x00FF0000;
        Ctx.BeaconCtx.BeaconTime.Seconds |=
            ((uint32_t)(payload[phyParam.BeaconFormat.Rfu1Size + 4] << 24)) &
            0xFF000000;
        Ctx.BeaconCtx.BeaconTime.SubSeconds = 0;
        Ctx.LoRaMacClassBParams.MlmeIndication->BeaconInfo.Time =
            Ctx.BeaconCtx.BeaconTime;
        beaconProcessed = true;
      }

      // Read CRC2 field from the frame
      beaconCrc1 = ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4 +
                                      2 + 7 + phyParam.BeaconFormat.Rfu2Size]) &
                   0x00FF;
      beaconCrc1 |=
          ((uint16_t)payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4 + 2 + 7 +
                             phyParam.BeaconFormat.Rfu2Size + 1]
           << 8) &
          0xFF00;
      crc1 = BeaconCrc(&payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4 + 2],
                       7 + phyParam.BeaconFormat.Rfu2Size);
      ...
      if (crc1 == beaconCrc1) {
        ...
        Ctx.LoRaMacClassBParams.MlmeIndication->BeaconInfo.GwSpecific.InfoDesc =
            payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4 + 2];
        memcpy1(
            Ctx.LoRaMacClassBParams.MlmeIndication->BeaconInfo.GwSpecific.Info,
            &payload[phyParam.BeaconFormat.Rfu1Size + 1 + 4 + 2 + 1], 6);
      }
      ...
      if (beaconProcessed == true) {
        ...
        TimerTime_t time = Radio.TimeOnAir(
            MODEM_LORA, bandwith, spreadingFactor, 1, 10, true, size, false);
        SysTime_t timeOnAir;
    ...
    // downlink except the beacon frame itself. The reason for this is that no
    // valid downlink window is open. If it receives a frame which is
    // 1. not a beacon or
    // 2. a beacon with a crc fail
    // the MAC shall ignore the frame completely. Thus, the function must always
    // return true, even if no valid beacon has been received.
    beaconProcessed = true;
  }
...
void LoRaMacClassBSetPingSlotInfo(uint8_t periodicity) {
  ...
  ClassBNvm->PingSlotCtx.PingPeriod =
      CalcPingPeriod(ClassBNvm->PingSlotCtx.PingNb);
#endif // LORAMAC_CLASSB_ENABLED
}
...
#ifdef LORAMAC_CLASSB_ENABLED
  if (nextClass == CLASS_B) { // Switch to from class a to class b
    if ((Ctx.BeaconCtx.Ctrl.BeaconMode == 1) &&
        (ClassBNvm->PingSlotCtx.Ctrl.Assigned == 1)) {
      return LORAMAC_STATUS_OK;
    }
  }
  if (nextClass == CLASS_A) { // Switch from class b to class a
...
uint8_t LoRaMacClassBPingSlotChannelReq(uint8_t datarate, uint32_t frequency) {
  ...
  if (frequency != 0) {
    ...
    if (RegionVerify(*Ctx.LoRaMacClassBParams.LoRaMacRegion, &verify,
                     PHY_FREQUENCY) == false) {
...
#ifdef LORAMAC_CLASSB_ENABLED
  Ctx.BeaconCtx.BeaconTimingDelay =
      (CLASSB_BEACON_DELAY_BEACON_TIMING_ANS * beaconTimingDelay);
  ...
  if (LoRaMacConfirmQueueIsCmdActive(MLME_BEACON_TIMING) == true) {
    if (Ctx.BeaconCtx.BeaconTimingDelay > CLASSB_BEACON_INTERVAL) {
      ...
      Ctx.BeaconCtx.NextBeaconRx =
          SysTimeFromMs(lastRxDone + Ctx.BeaconCtx.BeaconTimingDelay);
      LoRaMacConfirmQueueSetStatus(LORAMAC_EVENT_INFO_STATUS_OK,
                                   MLME_BEACON_TIMING);
    }

    Ctx.LoRaMacClassBParams.MlmeConfirm->BeaconTimingDelay =
        Ctx.BeaconCtx.BeaconTimingDelay;
    Ctx.LoRaMacClassBParams.MlmeConfirm->BeaconTimingChannel =
        Ctx.BeaconCtx.BeaconTimingChannel;
  }
#endif // LORAMAC_CLASSB_ENABLED
}
...
void LoRaMacClassBDeviceTimeAns(void) {
  ...
  Ctx.BeaconCtx.LastBeaconRx = SysTimeSub(
      Ctx.BeaconCtx.NextBeaconRx,
      (SysTime_t){.Seconds = CLASSB_BEACON_INTERVAL / 1000, .SubSeconds = 0});
  ...
  if (LoRaMacConfirmQueueIsCmdActive(MLME_DEVICE_TIME) == true) {
    if (currentTimeMs > SysTimeToMs(Ctx.BeaconCtx.NextBeaconRx)) {
      ...
      Ctx.BeaconCtx.Ctrl.BeaconDelaySet = 1;
      Ctx.BeaconCtx.BeaconTimingDelay =
          SysTimeToMs(Ctx.BeaconCtx.NextBeaconRx) - currentTimeMs;
      Ctx.BeaconCtx.BeaconTime.Seconds =
          nextBeacon.Seconds - UNIX_GPS_EPOCH_OFFSET - 128;
      Ctx.BeaconCtx.BeaconTime.SubSeconds = 0;
      LoRaMacConfirmQueueSetStatus(LORAMAC_EVENT_INFO_STATUS_OK,
                                   MLME_DEVICE_TIME);
    }
...
TimerTime_t LoRaMacClassBIsUplinkCollision(TimerTime_t txTimeOnAir) {
  ...
  beaconReserved = nextBeacon - CLASSB_BEACON_GUARD -
  ...
  if ((currentTime >= beaconReserved) &&
      (currentTime <
       (nextBeacon + CLASSB_BEACON_RESERVED))) { // Next beacon will be sent
                                                 // during the next uplink.
...
void LoRaMacClassBSetMulticastPeriodicity(MulticastCtx_t *multicastChannel) {
  ...
  if (multicastChannel != NULL) {
    multicastChannel->PingNb =
        CalcPingNb(multicastChannel->ChannelParams.RxParams.ClassB.Periodicity);

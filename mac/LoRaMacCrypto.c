...
/*
 * RJcount0 is a counter incremented with every Type 0 or 2 Rejoin frame
 * transmitted.
 */
 ...
 */
static LoRaMacCryptoStatus_t PayloadEncrypt(uint8_t *buffer, int16_t size,
  ...
  while (size > 0) {
    aBlock[15] = ctr & 0xFF;
    ctr++;
    if (SecureElementAesEncrypt(aBlock, 16, keyID, sBlock) !=
 ...
 * \retval                      - Status of the operation
 */
static LoRaMacCryptoStatus_t FOptsEncrypt(uint16_t size, uint32_t address,
                                          uint8_t dir, FCntIdentifier_t fCntID,
                                          uint32_t frameCounter,
                                          uint8_t *buffer) {
  if (buffer == 0) {
    return LORAMAC_CRYPTO_ERROR_NPE;
  ...
  if (size > 0) {
    if (SecureElementAesEncrypt(aBlock, 16, NWK_S_ENC_KEY, sBlock) !=
        SECURE_ELEMENT_SUCCESS) {
      return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
 ...
 * \param[IN]  keyID          - Key identifier
 * \param[IN]  isAck          - True if it is a acknowledge frame ( Sets
 * ConfFCnt in B0 block ) \param[IN]  devAddr        - Device address \param[IN]
 * dir            - Frame direction ( Uplink:0, Downlink:1 ) \param[IN]  fCnt -
 * Frame counter \param[IN/OUT]  b0         - B0 block \retval - Status of the
 * operation
 */
static LoRaMacCryptoStatus_t PrepareB0(uint16_t msgLen, KeyIdentifier_t keyID,
                                       bool isAck, uint8_t dir,
  ...
  if ((isAck == true) && (dir == DOWNLINK)) {
    // confFCnt contains the frame counter value modulo 2^16 of the "confirmed"
    // uplink or downlink frame that is being acknowledged
    uint16_t confFCnt = 0;
 ...
 * ConfFCnt in B0 block ) \param[IN]  devAddr        - Device address \param[IN]
 * dir            - Frame direction ( Uplink:0, Downlink:1 ) \param[IN]  fCnt -
 * Frame counter \param[OUT] cmac           - Computed cmac \retval - Status of
 * the operation
 */
static LoRaMacCryptoStatus_t ComputeCmacB0(uint8_t *msg, uint16_t len,
                                           KeyIdentifier_t keyID, bool isAck,
                                           uint8_t dir, uint32_t devAddr,
                                           uint32_t fCnt, uint32_t *cmac) {
  if ((msg == 0) || (cmac == 0)) {
    return LORAMAC_CRYPTO_ERROR_NPE;
  }
  ...
  if (SecureElementComputeAesCmac(micBuff, msg, len, keyID, cmac) !=
      SECURE_ELEMENT_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
  }
  return LORAMAC_CRYPTO_SUCCESS;
 ...
 * ConfFCnt in B0 block ) \param[IN]  devAddr        - Device address \param[IN]
 * dir            - Frame direction ( Uplink:0, Downlink:1 ) \param[IN]  fCnt -
 * Frame counter \param[in]  expectedCmac   - Expected cmac \retval - Status of
 * the operation
 */
static LoRaMacCryptoStatus_t VerifyCmacB0(uint8_t *msg, uint16_t len,
                                          KeyIdentifier_t keyID, bool isAck,
                                          uint8_t dir, uint32_t devAddr,
                                          uint32_t fCnt,
  ...
  retval = SecureElementVerifyAesCmac(micBuff, (len + MIC_BLOCK_BX_SIZE),
                                      expectedCmac, keyID);
 ...
 * \param[IN]  keyID          - Key identifier
 * \param[IN]  isAck          - True if it is a acknowledge frame ( Sets
 * ConfFCnt in B0 block ) \param[IN]  txDr           - Data rate used for the
 * transmission \param[IN]  txCh           - Index of the channel used for the
 * transmission \param[IN]  devAddr        - Device address \param[IN]  fCntUp
 * - Frame counter \param[IN/OUT]  b0         - B0 block \retval - Status of the
 * operation
 */
static LoRaMacCryptoStatus_t PrepareB1(uint16_t msgLen, KeyIdentifier_t keyID,
                                       bool isAck, uint8_t txDr, uint8_t txCh,
                                       uint32_t devAddr, uint32_t fCntUp,
                                       uint8_t *b1) {
  if (b1 == 0) {
    return LORAMAC_CRYPTO_ERROR_NPE;
  }
  ...
  if (isAck == true) {
    // confFCnt contains the frame counter value modulo 2^16 of the "confirmed"
    // uplink frame that is being acknowledged
...
/*
 * Computes cmac with adding B1 block in front ( only for Uplink frames
 * LoRaWAN 1.1 )
 *
 ...
 * transmission \param[IN]  txCh           - Index of the channel used for the
 * transmission \param[IN]  devAddr        - Device address \param[IN]  fCntUp
 * - Uplink Frame counter \param[OUT] cmac           - Computed cmac \retval -
 * Status of the operation
 */
static LoRaMacCryptoStatus_t ComputeCmacB1(uint8_t *msg, uint16_t len,
                                           KeyIdentifier_t keyID, bool isAck,
                                           uint8_t txDr, uint8_t txCh,
                                           uint32_t devAddr, uint32_t fCntUp,
                                           uint32_t *cmac) {
  if ((msg == 0) || (cmac == 0)) {
    return LORAMAC_CRYPTO_ERROR_NPE;
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t GetKeyAddrItem(AddressIdentifier_t addrID,
                                            KeyAddr_t **item) {
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t DeriveSessionKey10x(KeyIdentifier_t keyID,
                                                 uint32_t joinNonce,
                                                 uint32_t netID,
                                                 uint16_t devNonce) {
  ...
  if (SecureElementDeriveAndStoreKey(compBase, NWK_KEY, keyID) !=
      SECURE_ELEMENT_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
  }
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t DeriveSessionKey11x(KeyIdentifier_t keyID,
                                                 uint32_t joinNonce,
                                                 uint8_t *joinEUI,
                                                 uint16_t devNonce) {
  if (joinEUI == 0) {
 ...
 * \retval                    - Status of the operation
 */
static LoRaMacCryptoStatus_t DeriveLifeTimeSessionKey(KeyIdentifier_t keyID,
                                                      uint8_t *devEUI) {
  if (devEUI == 0) {
    return LORAMAC_CRYPTO_ERROR_NPE;
  }
 ...
 * \retval                     - Status of the operation
 */
static LoRaMacCryptoStatus_t GetLastFcntDown(FCntIdentifier_t fCntID,
                                             uint32_t *lastDown) {
  if (lastDown == NULL) {
    return LORAMAC_CRYPTO_ERROR_NPE;
  }
...
LoRaMacCryptoStatus_t LoRaMacCryptoSetKey(KeyIdentifier_t keyID, uint8_t *key) {
  ...
  if (keyID == APP_KEY) {
    ...
    if (LoRaMacCryptoDeriveMcRootKey(CryptoNvm->LrWanVersion.Fields.Minor,
                                     keyID) != LORAMAC_CRYPTO_SUCCESS) {
      return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
    }
...
LoRaMacCryptoPrepareJoinRequest(LoRaMacMessageJoinRequest_t *macMsg) {
  ...
  if (DeriveLifeTimeSessionKey(J_S_INT_KEY, macMsg->DevEUI) !=
      LORAMAC_CRYPTO_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR;
  }
  if (DeriveLifeTimeSessionKey(J_S_ENC_KEY, macMsg->DevEUI) !=
      LORAMAC_CRYPTO_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR;
  }
#endif
  ...
  if (SecureElementComputeAesCmac(
          NULL, macMsg->Buffer,
          (LORAMAC_JOIN_REQ_MSG_SIZE - LORAMAC_MIC_FIELD_SIZE),
          micComputationKeyID, &macMsg->MIC) != SECURE_ELEMENT_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
  }
...
LoRaMacCryptoPrepareReJoinType1(LoRaMacMessageReJoinType1_t *macMsg) {
  ...
  // Compute mic
  // cmac = aes128_cmac(JSIntKey, MHDR | RejoinType | JoinEUI| DevEUI |
  // RJcount1)
  if (SecureElementComputeAesCmac(
          NULL, macMsg->Buffer,
          (LORAMAC_RE_JOIN_1_MSG_SIZE - LORAMAC_MIC_FIELD_SIZE), J_S_INT_KEY,
          &macMsg->MIC) != SECURE_ELEMENT_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
  }
...
LoRaMacCryptoPrepareReJoinType0or2(LoRaMacMessageReJoinType0or2_t *macMsg) {
  ...
  if (SecureElementComputeAesCmac(
          NULL, macMsg->Buffer,
          (LORAMAC_RE_JOIN_0_2_MSG_SIZE - LORAMAC_MIC_FIELD_SIZE),
          S_NWK_S_INT_KEY, &macMsg->MIC) != SECURE_ELEMENT_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
  }
...
LoRaMacCryptoHandleJoinAccept(JoinReqIdentifier_t joinReqType, uint8_t *joinEUI,
  ...
  if (joinReqType == JOIN_REQ) {
    ...
    // If Join-accept is a reply to a rejoin, the RJcount(0 or 1) replaces
  ...
  if (SecureElementProcessJoinAccept(
          joinReqType, joinEUI, nonce, macMsg->Buffer, macMsg->BufSize,
          decJoinAccept, &versionMinor) != SECURE_ELEMENT_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
  }
  ...
  if (versionMinor == 1) {
    ...
    retval =
        DeriveSessionKey11x(F_NWK_S_INT_KEY, currentJoinNonce, joinEUI, nonce);
    if (retval != LORAMAC_CRYPTO_SUCCESS) {
      return retval;
    }

    retval =
        DeriveSessionKey11x(S_NWK_S_INT_KEY, currentJoinNonce, joinEUI, nonce);
    if (retval != LORAMAC_CRYPTO_SUCCESS) {
      return retval;
    }

    retval =
        DeriveSessionKey11x(NWK_S_ENC_KEY, currentJoinNonce, joinEUI, nonce);
    if (retval != LORAMAC_CRYPTO_SUCCESS) {
      return retval;
    }
...
#endif
  {
    ...
    retval =
        DeriveSessionKey10x(F_NWK_S_INT_KEY, currentJoinNonce, netID, nonce);
    if (retval != LORAMAC_CRYPTO_SUCCESS) {
      return retval;
    }

    retval =
        DeriveSessionKey10x(S_NWK_S_INT_KEY, currentJoinNonce, netID, nonce);
    if (retval != LORAMAC_CRYPTO_SUCCESS) {
      return retval;
    }
  }
...
LoRaMacCryptoStatus_t LoRaMacCryptoSecureMessage(uint32_t fCntUp, uint8_t txDr,
  ...
  if (fCntUp > CryptoNvm->FCntList.FCntUp) {
    retval = PayloadEncrypt(macMsg->FRMPayload, macMsg->FRMPayloadSize,
                            payloadDecryptionKeyID, macMsg->FHDR.DevAddr,
                            UPLINK, fCntUp);
    if (retval != LORAMAC_CRYPTO_SUCCESS) {
      return retval;
    }
    ...
    if (CryptoNvm->LrWanVersion.Fields.Minor == 1) {
      // Encrypt FOpts
      retval =
          FOptsEncrypt(macMsg->FHDR.FCtrl.Bits.FOptsLen, macMsg->FHDR.DevAddr,
                       UPLINK, FCNT_UP, fCntUp, macMsg->FHDR.FOpts);
      if (retval != LORAMAC_CRYPTO_SUCCESS) {
        return retval;
      }
    }
  ...
  if (CryptoNvm->LrWanVersion.Fields.Minor == 1) {
    ...
    retval = ComputeCmacB1(macMsg->Buffer,
                           (macMsg->BufSize - LORAMAC_MIC_FIELD_SIZE),
                           S_NWK_S_INT_KEY, macMsg->FHDR.FCtrl.Bits.Ack, txDr,
                           txCh, macMsg->FHDR.DevAddr, fCntUp, &cmacS);
    if (retval != LORAMAC_CRYPTO_SUCCESS) {
      return retval;
    }
    // cmacF = aes128_cmac(FNwkSIntKey, B0 | msg)
    retval = ComputeCmacB0(macMsg->Buffer,
                           (macMsg->BufSize - LORAMAC_MIC_FIELD_SIZE),
                           F_NWK_S_INT_KEY, macMsg->FHDR.FCtrl.Bits.Ack, UPLINK,
                           macMsg->FHDR.DevAddr, fCntUp, &cmacF);
    if (retval != LORAMAC_CRYPTO_SUCCESS) {
      return retval;
    }
  ...
  {
    // MIC = cmacF[0..3]
    // The IsAck parameter is every time false since the ConfFCnt field is not
    // used in legacy mode.
    retval = ComputeCmacB0(macMsg->Buffer,
                           (macMsg->BufSize - LORAMAC_MIC_FIELD_SIZE),
                           NWK_S_ENC_KEY, false, UPLINK, macMsg->FHDR.DevAddr,
                           fCntUp, &macMsg->MIC);
    if (retval != LORAMAC_CRYPTO_SUCCESS) {
      return retval;
    }
...
LoRaMacCryptoUnsecureMessage(AddressIdentifier_t addrID, uint32_t address,
  ...
  if (CryptoNvm->LrWanVersion.Fields.Minor == 0) {
    // In legacy mode the IsAck parameter is forced to be false since the
    // ConfFCnt field is not used.
    isAck = false;
  }

  // Verify mic
  retval = VerifyCmacB0(
      macMsg->Buffer, (macMsg->BufSize - LORAMAC_MIC_FIELD_SIZE),
      micComputationKeyID, isAck, DOWNLINK, address, fCntDown, macMsg->MIC);
  if (retval != LORAMAC_CRYPTO_SUCCESS) {
    return retval;
  }
  ...
  retval = PayloadEncrypt(macMsg->FRMPayload, macMsg->FRMPayloadSize,
                          payloadDecryptionKeyID, address, DOWNLINK, fCntDown);
  if (retval != LORAMAC_CRYPTO_SUCCESS) {
    return retval;
  }
  ...
  if (CryptoNvm->LrWanVersion.Fields.Minor == 1) {
    if (addrID == UNICAST_DEV_ADDR) {
      // Decrypt FOpts
      retval = FOptsEncrypt(macMsg->FHDR.FCtrl.Bits.FOptsLen, address, DOWNLINK,
                            fCntID, fCntDown, macMsg->FHDR.FOpts);
      if (retval != LORAMAC_CRYPTO_SUCCESS) {
        return retval;
      }
    }
...
LoRaMacCryptoStatus_t LoRaMacCryptoDeriveMcRootKey(uint8_t versionMinor,
  ...
  if (SecureElementDeriveAndStoreKey(compBase, keyID, MC_ROOT_KEY) !=
      SECURE_ELEMENT_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
  }
...
LoRaMacCryptoStatus_t LoRaMacCryptoDeriveMcKEKey(KeyIdentifier_t keyID) {
  ...
  if (SecureElementDeriveAndStoreKey(compBase, keyID, MC_KE_KEY) !=
      SECURE_ELEMENT_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
  }
...
LoRaMacCryptoDeriveMcSessionKeyPair(AddressIdentifier_t addrID,
  ...
  if (SecureElementDeriveAndStoreKey(compBaseAppS, curItem->RootKey,
                                     curItem->AppSkey) !=
      SECURE_ELEMENT_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
  }

  if (SecureElementDeriveAndStoreKey(compBaseNwkS, curItem->RootKey,
                                     curItem->NwkSkey) !=
      SECURE_ELEMENT_SUCCESS) {
    return LORAMAC_CRYPTO_ERROR_SECURE_ELEMENT_FUNC;
  }

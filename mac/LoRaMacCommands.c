...
Maintainer: Miguel Luis ( Semtech ), Daniel Jaeckle ( STACKFORCE ), Johannes
Bruder ( STACKFORCE )
*/
#include <stddef.h>
 ...
 *
 * \param[IN]     list           - List
 * \param[IN]     element        - Element where the previous element shall be
 * searched \retval                       - Status of the operation
 */
static MacCommand_t *LinkedListGetPrevious(MacCommandsList_t *list,
                                           MacCommand_t *element) {
  if ((list == NULL) || (element == NULL)) {
    return NULL;
  }
  ...
  // When current element is the first of the list, there's no previous element
  // so we can return NULL immediately.
  if (element != curElement) {
    // Loop through all elements until the end is reached or the next of current

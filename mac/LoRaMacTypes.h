 ...
 * \file      LoRaMacTypes.h
 *
 * \brief     LoRa MAC layer internal types definition. Please do not include in
 * application sources.
 *
...
/*!
 * LoRaWAN Frame type enumeration to differ between the possible data up/down
...
typedef enum eFType {
   ...
   * Frame type C
   *
   * FOptsLen = 0, Fopt not present, FPort = 0 , FRMPayload containing MAC
   * commands
   */
  FRAME_TYPE_C,
  /*!
...
typedef enum eFCntIdentifier {
  ...
  FCNT_UP = 0,
  /*!
   * Network downlink frame counter which is incremented with each downlink on
   * FPort 0 or when the FPort field is missing.
   */
  N_FCNT_DOWN,
  /*!
  ...
  MC_FCNT_DOWN_3,
  /*!
   * RJcount1 is a counter incremented with every transmitted  Type 1 Rejoin
   * request.
   */
  RJ_COUNT_0,
  /*!
   * RJcount0 is a counter incremented with every transmitted Type 0 or 2 Rejoin
   * request.
   */
  RJ_COUNT_1,
} FCntIdentifier_t;

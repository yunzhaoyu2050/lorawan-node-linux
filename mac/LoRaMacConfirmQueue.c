...
static MlmeConfirmQueue_t *
IncreaseBufferPointer(MlmeConfirmQueue_t *bufferPointer) {
  if (bufferPointer ==
      &ConfirmQueueCtx.Nvm
           .MlmeConfirmQueue[LORA_MAC_MLME_CONFIRM_QUEUE_LEN - 1]) {
...
static MlmeConfirmQueue_t *
DecreaseBufferPointer(MlmeConfirmQueue_t *bufferPointer) {
  if (bufferPointer == ConfirmQueueCtx.Nvm.MlmeConfirmQueue) {
    // Reset to the last element
    bufferPointer = &ConfirmQueueCtx.Nvm
                         .MlmeConfirmQueue[LORA_MAC_MLME_CONFIRM_QUEUE_LEN - 1];
  } else {
    bufferPointer--;
  }
...
static MlmeConfirmQueue_t *GetElement(Mlme_t request,
  ...
  for (uint8_t elementCnt = 0;
       elementCnt < ConfirmQueueCtx.Nvm.MlmeConfirmQueueCnt; elementCnt++) {
...
void LoRaMacConfirmQueueInit(LoRaMacPrimitives_t *primitives) {
  ...
  memset1((uint8_t *)ConfirmQueueCtx.Nvm.MlmeConfirmQueue, 0xFF,
...
bool LoRaMacConfirmQueueRemoveFirst(void) {
  ...
  ConfirmQueueCtx.Nvm.MlmeConfirmQueueCnt--;
  // Update start pointer
  ConfirmQueueCtx.BufferStart =
      IncreaseBufferPointer(ConfirmQueueCtx.BufferStart);
...
void LoRaMacConfirmQueueSetStatus(LoRaMacEventInfoStatus_t status,
  ...
  if (IsListEmpty(ConfirmQueueCtx.Nvm.MlmeConfirmQueueCnt) == false) {
    element = GetElement(request, ConfirmQueueCtx.BufferStart,
                         ConfirmQueueCtx.BufferEnd);
    if (element != NULL) {
...
bool LoRaMacConfirmQueueIsCmdActive(Mlme_t request) {
  if (GetElement(request, ConfirmQueueCtx.BufferStart,
                 ConfirmQueueCtx.BufferEnd) != NULL) {
    return true;
  }
  return false;
}

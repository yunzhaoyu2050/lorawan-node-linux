 ...
 *            - LoRaWAN regions can be activated by defining the related
 * preprocessor definition. It is possible to define more than one region. The
...
typedef struct sRxConfigParams {
   ...
   * ACTIVATION_TYPE_ABP or ACTIVATION_TYPE_OTTA )
   *
 ...
 * TRxEarly = 8 - DEFAULT_MIN_RX_SYMBOLS * tSymbol - RxWindowTimeout -
 *RADIO_WAKEUP_TIME
 *
 ...
 *
 * RxWindowTimeout = ( 2 * DEFAULT_MIN_RX_SYMBOLS - 8 ) * tSymbol + 2 *
 *DEFAULT_SYSTEM_MAX_RX_ERROR RxOffset = 4 * tSymbol - RxWindowTimeout / 2 -
 *RADIO_WAKE_UP_TIME
 *
 * Minimal value of RxWindowTimeout must be 5 symbols which implies that the
 *system always tolerates at least an error of 1.5 * tSymbol
 */
/*!
 ...
 * case, the end node shall ignore the command.
 */

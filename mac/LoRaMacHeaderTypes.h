...
#define LORAMAC_JOIN_REQ_MSG_SIZE                                              \
  (LORAMAC_MHDR_FIELD_SIZE + LORAMAC_JOIN_EUI_FIELD_SIZE +                     \
...
#define LORAMAC_RE_JOIN_1_MSG_SIZE                                             \
  (LORAMAC_MHDR_FIELD_SIZE + LORAMAC_JOIN_TYPE_FIELD_SIZE +                    \
   LORAMAC_JOIN_EUI_FIELD_SIZE + LORAMAC_DEV_EUI_FIELD_SIZE +                  \
   LORAMAC_RJCOUNT_1_FIELD_SIZE + LORAMAC_MIC_FIELD_SIZE)
...
#define LORAMAC_RE_JOIN_0_2_MSG_SIZE                                           \
  (LORAMAC_MHDR_FIELD_SIZE + LORAMAC_JOIN_TYPE_FIELD_SIZE +                    \
   ...
   LORAMAC_RJCOUNT_0_FIELD_SIZE + LORAMAC_MIC_FIELD_SIZE)

/*!
 * JoinAccept frame minimum size
 *
 * MHDR(1) + AppNonce(3) + NetID(3) + DevAddr(4) + DLSettings(1) + RxDelay(1) +
 * MIC(4)
 */
#define LORAMAC_JOIN_ACCEPT_FRAME_MIN_SIZE                                     \
  (LORAMAC_MHDR_FIELD_SIZE + LORAMAC_JOIN_NONCE_FIELD_SIZE +                   \
...
#define JOIN_ACCEPT_MIC_COMPUTATION_OFFSET                                     \
  (LORAMAC_MHDR_FIELD_SIZE + LORAMAC_JOIN_TYPE_FIELD_SIZE +                    \
   LORAMAC_JOIN_EUI_FIELD_SIZE + LORAMAC_DEV_NONCE_FIELD_SIZE)

/*!
 * FRMPayload overhead to be used when setting the Radio.SetMaxPayloadLength
 *
 * Overhead to be used when setting the Radio.SetMaxPayloadLength in
 * RxWindowSetup function.
 *
 * MHDR(1) + FHDR(7) + Port(1) + MIC(4)
 *
 * Maximum PHYPayload = MaxPayloadOfDatarate +
 * LORAMAC_FRAME_PAYLOAD_OVERHEAD_SIZE
 */
#define LORAMAC_FRAME_PAYLOAD_OVERHEAD_SIZE                                    \
  (LORAMAC_MHDR_FIELD_SIZE +                                                   \
   (LORAMAC_FHDR_DEV_ADDR_FIELD_SIZE + LORAMAC_FHDR_F_CTRL_FIELD_SIZE +        \

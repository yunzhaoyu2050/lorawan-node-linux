...
#if defined(LORA_RADIO_DRIVER_USING_ON_RTOS_RT_THREAD) ||                      \
    defined(LORA_RADIO_DRIVER_USING_ON_RTOS_RT_THREAD_NANO)
...
//     rt_timer_init(&(obj->timer),name,(void
//     (*)(void*))callback,RT_NULL,1000,RT_TIMER_FLAG_ONE_SHOT|RT_TIMER_FLAG_SOFT_TIMER);
// }
...
#endif // End Of  ( LORA_RADIO_DRIVER_USING_ON_RTOS_RT_THREAD ) || defined (
       // LORA_RADIO_DRIVER_USING_ON_RTOS_RT_THREAD_NANO )

uint32_t GetElapsedTime(struct timespec *last_ts) {
  struct timespec now_ts = {0};
  clock_gettime(CLOCK_MONOTONIC, &now_ts);
  return (uint32_t)ts_diff(&now_ts, last_ts);
} // rtick_timer_get_elapsed_time

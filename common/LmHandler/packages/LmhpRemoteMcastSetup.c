 ...
 */
static void LmhpRemoteMcastSetupInit(void *params, uint8_t *dataBuffer,
                                     uint8_t dataBufferMaxSize);
 ...
 */
static void
LmhpRemoteMcastSetupOnMcpsIndication(McpsIndication_t *mcpsIndication);
...
typedef enum eSessionState { SESSION_STOPED, SESSION_STARTED } SessionState_t;
...
static void LmhpRemoteMcastSetupProcess(void) {
  ...
  LmhpRemoteMcastSetupState.SessionState =
      REMOTE_MCAST_SETUP_SESSION_STATE_IDLE;
  ...
  case REMOTE_MCAST_SETUP_SESSION_STATE_START:
    ...
    TimerSetValue(&SessionStopTimer, 0,
                  (1 << McSessionData[0].SessionTimeout) * 1000);
...
LmhpRemoteMcastSetupOnMcpsIndication(McpsIndication_t *mcpsIndication) {
  ...
  while (cmdIndex < mcpsIndication->BufferSize) {
    ...
    case REMOTE_MCAST_SETUP_PKG_VERSION_REQ: {
      LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++] =
          REMOTE_MCAST_SETUP_PKG_VERSION_ANS;
      LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++] =
          REMOTE_MCAST_SETUP_ID;
      LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++] =
          REMOTE_MCAST_SETUP_VERSION;
      break;
    }
    ...
    case REMOTE_MCAST_SETUP_MC_GROUP_SETUP_REQ: {
      ...
      for (int8_t i = 0; i < 16; i++) {
        McSessionData[id].McGroupData.McKeyEncrypted[i] =
      ...
      McChannelParams_t channel = {
          ...
          .Class = CLASS_C, // Field not used for multicast channel setup. Must
                            // be initialized to something
          .IsEnabled = true,
          .GroupID = (AddressIdentifier_t)McSessionData[id]
                         .McGroupData.IdHeader.Fields.McGroupId,
          ...
          .RxParams.ClassC = // Field not used for multicast channel setup. Must
                             // be initialized to something
          {.Frequency = 0, .Datarate = 0}};
    ...
    case REMOTE_MCAST_SETUP_MC_GROUP_DELETE_REQ: {
      ...
      LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++] =
          REMOTE_MCAST_SETUP_MC_GROUP_DELETE_ANS;

      if (LoRaMacMcChannelDelete((AddressIdentifier_t)id) !=
          LORAMAC_STATUS_OK) {
    ...
    case REMOTE_MCAST_SETUP_MC_GROUP_CLASS_C_SESSION_REQ: {
      ...
      McSessionData[id].SessionTimeout =
          mcpsIndication->Buffer[cmdIndex++] & 0x0F;
      ...
      LmhpRemoteMcastSetupState.DataBuffer[dataBufferIndex++] =
          REMOTE_MCAST_SETUP_MC_GROUP_CLASS_C_SESSION_ANS;
      if (LoRaMacMcChannelSetupRxParams((AddressIdentifier_t)id,
                                        &McSessionData[id].RxParams,
                                        &status) == LORAMAC_STATUS_OK) {
        SysTime_t curTime = {.Seconds = 0, .SubSeconds = 0};
        curTime = SysTimeGet();

        int32_t timeToSessionStart =
  ...
  if (dataBufferIndex != 0) {
    // Answer commands
    LmHandlerAppData_t appData = {.Buffer =
                                      LmhpRemoteMcastSetupState.DataBuffer,

 ...
 */
static void LmhpFragmentationInit(void *params, uint8_t *dataBuffer,
                                  uint8_t dataBufferMaxSize);
...
static void LmhpFragmentationProcess(void) {
  ...
  // Set the state to idle so that the other states are executed only when they
  // are set in the appropriate functions.
...
static void
LmhpFragmentationOnMcpsIndication(McpsIndication_t *mcpsIndication) {
  uint8_t cmdIndex = 0;
  ...
  while (cmdIndex < mcpsIndication->BufferSize) {
    ...
    case FRAGMENTATION_FRAG_STATUS_REQ: {
      ...
      if ((participants == 1) ||
          ((participants == 0) &&
           (FragSessionData[fragIndex].FragDecoderStatus.FragNbLost > 0))) {
        LmhpFragmentationState.DataBuffer[dataBufferIndex++] =
            FRAGMENTATION_FRAG_STATUS_ANS;
        LmhpFragmentationState.DataBuffer[dataBufferIndex++] =
            FragSessionData[fragIndex].FragDecoderStatus.FragNbRx & 0xFF;
        LmhpFragmentationState.DataBuffer[dataBufferIndex++] =
            (fragIndex << 6) |
            ((FragSessionData[fragIndex].FragDecoderStatus.FragNbRx >> 8) &
             0x3F);
        LmhpFragmentationState.DataBuffer[dataBufferIndex++] =
            FragSessionData[fragIndex].FragDecoderStatus.FragNbLost;
        LmhpFragmentationState.DataBuffer[dataBufferIndex++] =
            FragSessionData[fragIndex].FragDecoderStatus.MatrixError & 0x01;

        // Fetch the co-efficient value required to calculate delay of that
        // respective session.
        blockAckDelay = FragSessionData[fragIndex]
                            .FragGroupData.Control.Fields.BlockAckDelay;
    ...
    case FRAGMENTATION_FRAG_SESSION_SETUP_REQ: {
      ...
      fragSessionData.FragGroupData.FragSession.Value =
          mcpsIndication->Buffer[cmdIndex++];

      fragSessionData.FragGroupData.FragNb =
          (mcpsIndication->Buffer[cmdIndex++] << 0) & 0x00FF;
      fragSessionData.FragGroupData.FragNb |=
          (mcpsIndication->Buffer[cmdIndex++] << 8) & 0xFF00;

      fragSessionData.FragGroupData.FragSize =
          mcpsIndication->Buffer[cmdIndex++];

      fragSessionData.FragGroupData.Control.Value =
          mcpsIndication->Buffer[cmdIndex++];

      fragSessionData.FragGroupData.Padding =
          mcpsIndication->Buffer[cmdIndex++];

      fragSessionData.FragGroupData.Descriptor =
          (mcpsIndication->Buffer[cmdIndex++] << 0) & 0x000000FF;
      fragSessionData.FragGroupData.Descriptor +=
          (mcpsIndication->Buffer[cmdIndex++] << 8) & 0x0000FF00;
      fragSessionData.FragGroupData.Descriptor +=
          (mcpsIndication->Buffer[cmdIndex++] << 16) & 0x00FF0000;
      fragSessionData.FragGroupData.Descriptor +=
          (mcpsIndication->Buffer[cmdIndex++] << 24) & 0xFF000000;
...
#endif
      status |=
          (fragSessionData.FragGroupData.FragSession.Fields.FragIndex << 6) &
          0xC0;
    ...
    case FRAGMENTATION_FRAG_SESSION_DELETE_REQ: {
      ...
      status |= id;
      if ((id >= FRAGMENTATION_MAX_SESSIONS) ||
      ...
      LmhpFragmentationState.DataBuffer[dataBufferIndex++] =
          FRAGMENTATION_FRAG_SESSION_DELETE_ANS;
      LmhpFragmentationState.DataBuffer[dataBufferIndex++] = status;
      isAnswerDelayed = false;
      break;
    }
    case FRAGMENTATION_DATA_FRAGMENT: {
      ...
      if (mcpsIndication->Multicast == 1) {
        ...
        // uint8_t groupId = LoRaMacMcChannelGetGroupId(
        // mcpsIndication->DevAddress ); if( ( groupId == 0xFF ) ||
        //    ( (
      ...
      if (FragSessionData[fragIndex].FragDecoderPorcessStatus ==
          FRAG_SESSION_ONGOING) {
        FragSessionData[fragIndex].FragDecoderPorcessStatus =
            FragDecoderProcess(fragCounter, &mcpsIndication->Buffer[cmdIndex]);
        FragSessionData[fragIndex].FragDecoderStatus = FragDecoderGetStatus();
        if (LmhpFragmentationParams->OnProgress != NULL) {
          LmhpFragmentationParams->OnProgress(
              FragSessionData[fragIndex].FragDecoderStatus.FragNbRx,
...
#if (FRAG_DECODER_FILE_HANDLING_NEW_API == 1)
            LmhpFragmentationParams->OnDone(
                FragSessionData[fragIndex].FragDecoderPorcessStatus,
                (FragSessionData[fragIndex].FragGroupData.FragNb *
                 FragSessionData[fragIndex].FragGroupData.FragSize) -
  ...
  // After processing the commands, if the end-node has to reply back then a
  // flag is checked if the reply is to be sent immediately or with a delay. In
  // some scenarios it is not desired that multiple end-notes send uplinks at
  // the same time to the same server. (Example: Fragment status during a
  // multicast FUOTA)
  if (dataBufferIndex != 0) {
    ...
    if (isAnswerDelayed == true) {
      // Delay value is calculated using BlockAckDelay which is communicated by
      // server during the FragSessionSetupReq Pseudo Random Delay = rand(0:1) *
      // 2^(blockAckDelay + 4) Seconds. Delay = Pseudo Random Delay * 1000 milli
      // seconds. Eg: blockAckDelay = 7

...
typedef struct LmHandlerCallbacks_s {
   ...
   * 通知上层已向 MAC 层发出 MCPS 请求
   * \param   [IN] status      - Request returned status
   * \param   [IN] mcpsRequest - Performed MCPS-Request. Refer to \ref
   * McpsReq_t. \param   [IN] nextTxDelay - Time to wait until another TX is
   * possible.
   */
   ...
   * 通知上层已向 MAC 层发出 MLME 请求
   * \param   [IN] status      - Request returned status
   * \param   [IN] mlmeRequest - Performed MLME-Request. Refer to \ref
   * MlmeReq_t. \param   [IN] nextTxDelay - Time to wait until another TX is
   * possible.
   */
   ...
   * 通知上层系统时间已更新
   * \param [in] isSynchronized Indicates if the system time is synchronized in
   * the range +/-1 second \param [in] timeCorrection Received time correction
   * value
   */
 ...
 *                         Example: 2^3 = 8 seconds. The end-device will open an
 * Rx slot every 8 seconds.
 *
 * \retval status Returns \ref LORAMAC_HANDLER_SUCCESS if request has been
 *                processed else \ref LORAMAC_HANDLER_ERROR
 */
 ...
 *
 * \Note Callback \ref LmHandlerConfirmClass informs upper layer that the change
 * has occurred \Note Only switch from class A to class B/C OR from class B/C to
 * class A is allowed
 *

...
void CayenneLppInit(void) { CayenneLppCursor = 0; }

void CayenneLppReset(void) { CayenneLppCursor = 0; }

uint8_t CayenneLppGetSize(void) { return CayenneLppCursor; }

uint8_t *CayenneLppGetBuffer(void) { return CayenneLppBuffer; }
...
uint8_t CayenneLppAddDigitalInput(uint8_t channel, uint8_t value) {
  if ((CayenneLppCursor + LPP_DIGITAL_INPUT_SIZE) >
...
uint8_t CayenneLppAddDigitalOutput(uint8_t channel, uint8_t value) {
  if ((CayenneLppCursor + LPP_DIGITAL_OUTPUT_SIZE) >
      CAYENNE_LPP_MAXBUFFER_SIZE) {
    return 0;
  }
  CayenneLppBuffer[CayenneLppCursor++] = channel;
...
uint8_t CayenneLppAddAnalogOutput(uint8_t channel, float value) {
  if ((CayenneLppCursor + LPP_ANALOG_OUTPUT_SIZE) >
      CAYENNE_LPP_MAXBUFFER_SIZE) {
    return 0;
  }
  int16_t val = (int16_t)(value * 100);
...
uint8_t CayenneLppAddRelativeHumidity(uint8_t channel, float rh) {
  if ((CayenneLppCursor + LPP_RELATIVE_HUMIDITY_SIZE) >
      CAYENNE_LPP_MAXBUFFER_SIZE) {
    return 0;
  }
  CayenneLppBuffer[CayenneLppCursor++] = channel;
...
uint8_t CayenneLppAddAccelerometer(uint8_t channel, float x, float y, float z) {
  if ((CayenneLppCursor + LPP_ACCELEROMETER_SIZE) >
      CAYENNE_LPP_MAXBUFFER_SIZE) {
    return 0;
  }
...
uint8_t CayenneLppAddBarometricPressure(uint8_t channel, float hpa) {
  if ((CayenneLppCursor + LPP_BAROMETRIC_PRESSURE_SIZE) >
      CAYENNE_LPP_MAXBUFFER_SIZE) {
    return 0;
  }

 ...
 * symbols) \param [IN] symbTimeout  Sets the RxSingle timeout value FSK :
 * timeout in number of bytes LoRa: timeout in symbols \param [IN] fixLen Fixed
 * length packets [0: variable, 1: fixed] \param [IN] payloadLen   Sets payload
 * length when fixed length is used \param [IN] crcOn        Enables/Disables
 * the CRC [0: OFF, 1: ON] \param [IN] FreqHopOn    Enables disables the
 * intra-packet frequency hopping FSK : N/A ( set to 0 ) LoRa: [0: OFF, 1: ON]
 ...
 *                          FSK : Number of bytes
 *                          LoRa: Length in symbols (the hardware adds 4 more
 * symbols) \param [IN] fixLen       Fixed length packets [0: variable, 1:
 * fixed] \param [IN] crcOn        Enables disables the CRC [0: OFF, 1: ON]
 ...
 *                          LoRa: Length in symbols (the hardware adds 4 more
 * symbols) \param [IN] fixLen       Fixed length packets [0: variable, 1:
 * fixed] \param [IN] payloadLen   Sets payload length when fixed length is used
...
/*!
 * \brief Gets the time required for the board plus radio to get out of
 * sleep.[ms]
 *
...
const struct Radio_s Radio = {
    RadioInit, RadioGetStatus, RadioSetModem, RadioSetChannel,
    RadioIsChannelFree, RadioRandom, RadioSetRxConfig, RadioSetTxConfig,
    RadioCheckRfFrequency, RadioTimeOnAir, RadioSend, RadioSleep, RadioStandby,
    RadioRx, RadioStartCad, RadioSetTxContinuousWave, RadioRssi, RadioWrite,
    RadioRead,
    // RadioWriteBuffer,
    // RadioReadBuffer,
    RadioSetMaxPayloadLength, RadioSetPublicNetwork, RadioGetWakeupTime,
    RadioIrqProcess, RadioCheck,
    // Available on SX126x only
    RadioRxBoosted, RadioSetRxDutyCycle};
 ...
 * \brief spi initilize
 */
// extern struct rt_spi_device *lora_radio_spi_init(const char *bus_name,const
// char *lora_device_name, rt_uint8_t param);
...
void RadioSetChannel(uint32_t freq) { SX126xSetRfFrequency(freq); }
...
uint32_t RadioRandom(void) {
  ...
  log(INFO, "RadioSetModem success.");
  // Disable LoRa modem interrupts ( RxDone RXTimeout )
  SX126xSetDioIrqParams(IRQ_RADIO_NONE, IRQ_RADIO_NONE, IRQ_RADIO_NONE,
                        IRQ_RADIO_NONE);
  log(INFO, "SX126xSetDioIrqParams success.");
  rnd = SX126xGetRandom();
  log(INFO, "SX126xGetRandom success.");
  return rnd;
}
...
void RadioSetRxConfig(RadioModems_t modem, uint32_t bandwidth,
  ...
  case MODEM_FSK:
    ...
    SX126x.PacketParams.Params.Gfsk.PreambleLength =
        (preambleLen << 3); // convert byte into bit
    ...
    RadioStandby();
    RadioSetModem((SX126x.ModulationParams.PacketType == PACKET_TYPE_GFSK)
                      ? MODEM_FSK
                      : MODEM_LORA);
    SX126xSetModulationParams(&SX126x.ModulationParams);
    SX126xSetPacketParams(&SX126x.PacketParams);
    SX126xSetSyncWord(
        (uint8_t[]){0xC1, 0x94, 0xC1, 0x00, 0x00, 0x00, 0x00, 0x00});
    SX126xSetWhiteningSeed(0x01FF);

    RxTimeout =
        (uint32_t)(symbTimeout * ((1.0 / (double)datarate) * 8.0) * 1000);
    break;
...
static uint32_t RadioGetGfskTimeOnAirNumerator(uint32_t datarate,
                                               uint8_t coderate,
  ...
  return (preambleLen << 3) + ((fixLen == false) ? 8 : 0) +
         (syncWordLength << 3) +
         ((payloadLen + (addrComp == RADIO_ADDRESSCOMP_FILT_OFF ? 0 : 1) +
           ((crcOn == true) ? 2 : 0))
          << 3);
}

static uint32_t
RadioGetLoRaTimeOnAirNumerator(uint32_t bandwidth, uint32_t datarate,
  ...
  int32_t ceilNumerator =
      (payloadLen << 3) + (crcOn ? 16 : 0) - (4 * datarate) + (fixLen ? 0 : 20);
  ...
  int32_t intermediate =
      ((ceilNumerator + ceilDenominator - 1) / ceilDenominator) * crDenom +
      preambleLen + 12;
...
uint32_t RadioTimeOnAir(RadioModems_t modem, uint32_t bandwidth,
  ...
  case MODEM_FSK: {
    numerator =
        1000U * RadioGetGfskTimeOnAirNumerator(datarate, coderate, preambleLen,
                                               fixLen, payloadLen, crcOn);
    denominator = datarate;
  } break;
  case MODEM_LORA: {
    numerator = 1000U * RadioGetLoRaTimeOnAirNumerator(
                            bandwidth, datarate, coderate, preambleLen, fixLen,
                            payloadLen, crcOn);
...
void RadioStandby(void) { SX126xSetStandby(STDBY_RC); }
...
int16_t RadioRssi(RadioModems_t modem) { return SX126xGetRssiInst(); }

void RadioWrite(uint16_t addr, uint8_t data) {
  SX126xWriteRegister(addr, data);
}

uint8_t RadioRead(uint16_t addr) { return SX126xReadRegister(addr); }
...
#endif
  {
    ...
    if ((irqRegs & IRQ_RX_DONE) == IRQ_RX_DONE) {
      if ((irqRegs & IRQ_CRC_ERROR) == IRQ_CRC_ERROR) {
        ...
        if (RxContinuous == false) {
          ...
          // WORKAROUND - Implicit Header Mode Timeout Behavior, see
          // DS_SX1261-2_V1.2 datasheet chapter 15.3 RegRtcControl = @address
          // 0x0902
    ...
    if ((irqRegs & IRQ_CAD_DONE) == IRQ_CAD_DONE) {
      ...
      if ((RadioEvents != NULL) && (RadioEvents->CadDone != NULL)) {
        RadioEvents->CadDone(((irqRegs & IRQ_CAD_ACTIVITY_DETECTED) ==
                              IRQ_CAD_ACTIVITY_DETECTED));
      }
      log(WARN, "PHY CAD Done\r");
    }

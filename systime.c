...
// #define UNIX_HOUR_OFFSET ((TM_DAYS_IN_LEAP_YEAR + TM_DAYS_IN_YEAR) *
// TM_SECONDS_IN_1DAY)
...
// static void CalendarDiv86400(uint32_t in, uint32_t *out, uint32_t
// *remainder); static uint32_t CalendarDiv61(uint32_t in); static void
// CalendarDiv60(uint32_t in, uint32_t *out, uint32_t *remainder);

// const char *WeekDayString[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri",
// "Sat"};
...
void SysTimeSet(SysTime_t sysTime) {
  ...
  st.tv_sec = sysTime.Seconds;
  st.tv_nsec = sysTime.SubSeconds * 1000000;
  clock_settime(CLOCK_REALTIME, &st);
}
...
uint32_t SysTimeToMs(SysTime_t sysTime) {
  ...
  // SysTime_t deltaTime = {.Seconds = seconds, .SubSeconds =
  // (int16_t)subSeconds};
...
SysTime_t SysTimeFromMs(uint32_t timeMs) {
  ...
  // SysTime_t sysTime = {.Seconds = seconds, .SubSeconds =
  // (int16_t)subSeconds};
  ...
  struct timespec req;
  req.tv_sec = (time_t)(timeMs / 1000);
  req.tv_nsec = (timeMs % 1000) * 1000000;

  SysTime_t c = {.Seconds = req.tv_sec, .SubSeconds = (req.tv_nsec * 1000000)};
  return c;
}

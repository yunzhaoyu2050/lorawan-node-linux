 ...
 */
static SecureElementStatus_t GetKeyByID(KeyIdentifier_t keyID,
                                        Key_t **keyItem) {
 ...
 */
static SecureElementStatus_t ComputeCmac(uint8_t *micBxBuffer, uint8_t *buffer,
...
SecureElementStatus_t SecureElementSetKey(KeyIdentifier_t keyID, uint8_t *key) {
  ...
  for (uint8_t i = 0; i < NUM_OF_KEYS; i++) {
    if (SeNvm->KeyList[i].KeyID == keyID) {
      if ((keyID == MC_KEY_0) || (keyID == MC_KEY_1) || (keyID == MC_KEY_2) ||
          (keyID == MC_KEY_3)) { // Decrypt the key if its a Mckey
...
SecureElementStatus_t SecureElementProcessJoinAccept(
  ...
  if ((encJoinAccept == NULL) || (decJoinAccept == NULL) ||
      (versionMinor == NULL)) {
    return SECURE_ELEMENT_ERROR_NPE;
  }
  ...
  if (SecureElementAesEncrypt(
          encJoinAccept + LORAMAC_MHDR_FIELD_SIZE,
          encJoinAcceptSize - LORAMAC_MHDR_FIELD_SIZE, encKeyID,
          decJoinAccept + LORAMAC_MHDR_FIELD_SIZE) != SECURE_ELEMENT_SUCCESS) {
  ...
  *versionMinor = ((decJoinAccept[11] & 0x80) == 0x80) ? 1 : 0;
  ...
  mic = ((uint32_t)decJoinAccept[encJoinAcceptSize - LORAMAC_MIC_FIELD_SIZE]
         << 0);
  mic |=
      ((uint32_t)decJoinAccept[encJoinAcceptSize - LORAMAC_MIC_FIELD_SIZE + 1]
       << 8);
  mic |=
      ((uint32_t)decJoinAccept[encJoinAcceptSize - LORAMAC_MIC_FIELD_SIZE + 2]
       << 16);
  mic |=
      ((uint32_t)decJoinAccept[encJoinAcceptSize - LORAMAC_MIC_FIELD_SIZE + 3]
       << 24);

  //  - Header buffer to be used for MIC computation
  //        - LoRaWAN 1.0.x : micHeader = [MHDR(1)]
  //        - LoRaWAN 1.1.x : micHeader = [JoinReqType(1), JoinEUI(8),
  //        DevNonce(2), MHDR(1)]

  // Verify mic
  if (*versionMinor == 0) {
    // For LoRaWAN 1.0.x
    //   cmac = aes128_cmac(NwkKey, MHDR |  JoinNonce | NetID | DevAddr |
    //   DLSettings | RxDelay | CFList | CFListType)
    if (SecureElementVerifyAesCmac(decJoinAccept,
                                   (encJoinAcceptSize - LORAMAC_MIC_FIELD_SIZE),
                                   mic, NWK_KEY) != SECURE_ELEMENT_SUCCESS) {
    ...
    //   CFListType)
    // Prepare the msg for integrity check (adding JoinReqType, JoinEUI and
    // DevNonce)
    uint8_t localBuffer[LORAMAC_JOIN_ACCEPT_FRAME_MAX_SIZE +
                        JOIN_ACCEPT_MIC_COMPUTATION_OFFSET] = {0};

    memcpy1(localBuffer, micHeader11, JOIN_ACCEPT_MIC_COMPUTATION_OFFSET);
    memcpy1(localBuffer + JOIN_ACCEPT_MIC_COMPUTATION_OFFSET - 1, decJoinAccept,
            encJoinAcceptSize);
...
uint8_t *SecureElementGetDevEui(void) { return SeNvm->DevEui; }
...
uint8_t *SecureElementGetJoinEui(void) { return SeNvm->JoinEui; }
...
uint8_t *SecureElementGetPin(void) { return SeNvm->Pin; }
